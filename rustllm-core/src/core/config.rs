...
impl PluginConfigManager {
    ...
    pub fn set_plugin_config(&self, plugin: &PluginName, config: ConfigValue) -> Result<()> {
        self.store
            .set(format!("plugin.{}", plugin.as_str()), config)
    }
    ...
    pub fn get_plugin_key(&self, plugin: &PluginName, key: &str) -> Result<Option<ConfigValue>> {
        let config_key = format!("plugin.{}.{}", plugin.as_str(), key);
