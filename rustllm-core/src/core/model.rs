...
impl Transformer250MConfig {
    ...
    pub fn num_parameters(&self) -> usize {
        ...
        let per_layer = {
            ...
            let qkv_bias = if self.use_bias { 3 * self.hidden_dim } else { 0 };
            ...
            let ff1_bias = if self.use_bias { self.intermediate_dim } else { 0 };
            ...
            qkv_params + qkv_bias + out_proj_params + out_proj_bias +
            ff1_params + ff1_bias + ff2_params + ff2_bias + ln_params
        };
    ...
    pub fn validate(&self) -> Result<()> {
        use crate::foundation::error::{Error, ConfigError};
