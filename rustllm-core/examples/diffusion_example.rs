...
impl DiffusionSampler for DDPMSampler {
    fn sample_step(
        ...
        let predicted_x0: Vec<f32> = current
            .iter()
            .zip(predicted_noise.iter())
            .map(|(&x_t, &noise)| (x_t - noise * (1.0 - alpha_bar).sqrt()) / alpha_bar.sqrt())
            .collect();
        ...
        // Sample next step
        let next_sample: Vec<f32> = predicted_x0
            .iter()
            .zip(current.iter())
            .map(|(&x0_pred, &x_t)| {
...
impl SimpleDiffusionModel {
    ...
    fn generate_samples(
        ...
        for _ in 0..num_samples {
            ...
            let mut current: Vec<f32> = (0..self.dim)
                .map(|i| ((i + 1) as f32 * 0.1).sin())
                .collect();
...
fn main() -> Result<()> {
    ...
    println!(
        "Noise schedule: {} steps, beta range [{:.4}, {:.4}]",
    ...
    let batch_results: Vec<Vec<f32>> = (0..5)
        ...
        .map(|initial_noise| {
            ...
            for timestep in (0..noise_schedule.num_steps()).rev() {
                current = model
                    .denoise_step(&current, timestep, &noise_schedule, &sampler)
    ...
    let view = SliceView::new(&data).map(|x| x * 2.0);

    println!(
        "Original data[0]: {}, Transformed view[0]: {:?}",
        data[0],
        view.get(0)
    );
