...
fn stream_process_file(path: &str) -> Result<()> {
    ...
    let file =
        File::open(path).map_err(|e| internal_error(format!("Failed to open file: {}", e)))?;
    ...
    for (line_num, line_result) in reader.lines().enumerate() {
        let line =
            line_result.map_err(|e| internal_error(format!("Failed to read line: {}", e)))?;
...
fn parallel_chunk_processing() -> Result<()> {
    ...
    let corpus: Vec<String> = (0..100)
        .map(|i| {
            format!(
                "This is sentence number {}. It contains some text for processing.",
                i
            )
        })
        .collect();
    ...
    for (chunk_idx, chunk) in corpus.chunks(chunk_size).enumerate() {
        // Process each chunk
        let chunk_tokens: Vec<Vec<_>> = chunk
            .iter()
            .map(|text| tokenizer.tokenize_str(text).collect::<Vec<_>>())
            .collect();

        let chunk_token_count: usize = chunk_tokens.iter().map(|tokens| tokens.len()).sum();
...
fn create_sample_file() -> Result<()> {
    ...
    let mut file =
        File::create(path).map_err(|e| internal_error(format!("Failed to create file: {}", e)))?;

    // Write 10,000 lines
    for i in 0..10_000 {
        writeln!(
            file,
            "This is line number {}. It contains sample text for streaming processing. \
