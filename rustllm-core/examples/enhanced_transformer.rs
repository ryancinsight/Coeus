...
fn main() -> Result<()> {
    ...
    println!("✓ Forward pass completed in {:.2}ms", inference_time.as_millis());
    println!("📊 Output shape: [{}] (vocabulary logits)", output.len());
    
    // Find top-k predictions
    let mut indexed_logits: Vec<(usize, f32)> = output.iter()
        .enumerate()
        .map(|(i, &logit)| (i, logit))
        .collect();
    ...
    println!("   • Parameters/Second: {:.1}M", 
             model.num_parameters() as f64 / inference_time.as_secs_f64() / 1_000_000.0);
    
    // Architecture highlights
    println!("\n🏗️  Architecture Highlights");
    println!("============================");
    println!("✓ Multi-Head Attention with {} heads", model.config().num_heads);
