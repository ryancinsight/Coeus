...
use rustllm_model_transformer::{TransformerModelBuilder, TransformerModelPlugin};
...
fn main() -> Result<()> {
    ...
    println!(
        "\nEstimated parameters: {} ({:.1}M)",
        param_count,
        param_count as f64 / 1_000_000.0
    );
    println!(
        "Memory required: {:.1} MB (float32)",
        param_count as f64 * 4.0 / 1_000_000.0
    );
    ...
    println!(
        "Inference time: {:.3}ms",
        inference_time.as_micros() as f64 / 1000.0
    );
    ...
    println!(
        "Loaded model inference successful, output shape: [{}]",
        test_output.len()
    );
