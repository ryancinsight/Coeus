...
fn main() -> Result<()> {
    ...
    let model_config = ConfigBuilder::map()
        .with("architecture", ConfigBuilder::map()
            .with("type", "transformer")
            .with("layers", 6)
            .with("attention", ConfigBuilder::map()
                .with("heads", 8)
                .with("dim_per_head", 64)
                .with("dropout", 0.1)
                .build())
            .build())
        .with("training", ConfigBuilder::map()
            .with("optimizer", "adam")
    ...
    if let ConfigValue::Map(root) = model_config {
        if let Some(ConfigValue::Map(arch)) = root.get("architecture") {
            println!("Architecture type: {:?}", arch.get("type").and_then(|v| v.as_string()));
            
            if let Some(ConfigValue::Map(attention)) = arch.get("attention") {
                println!("Attention heads: {:?}", attention.get("heads").and_then(|v| v.as_integer()));
            }
        }
    }
