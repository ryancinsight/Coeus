...
use rustllm_core::core::model::{BasicModelConfig, ForwardModel, Model, ModelBuilder, ModelConfig};
use rustllm_core::core::plugin::{ModelBuilderPlugin, Plugin, PluginCapabilities};
use rustllm_core::core::traits::{Identity, Versioned};
use rustllm_core::foundation::{error::Result, types::Version};

/// Basic model plugin.
#[derive(Debug, Default)]
pub struct BasicModelPlugin;

impl Identity for BasicModelPlugin {
    fn id(&self) -> &str {
        "basic_model"
    }
}
...
impl Plugin for BasicModelPlugin {
    fn capabilities(&self) -> PluginCapabilities {
        PluginCapabilities::standard().with_feature("model_building")
    }
}
