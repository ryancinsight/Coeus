...
/// Asin function for automatic differentiation
#[derive(Debug)]
pub struct AsinFunction<B, S, T>
where
    B: Backend<Data = T>,
...
impl<B, S, T> Function<B, S, T> for AsinFunction<B, S, T>
where
    ...
    T: DataType + FloatExt + num_traits::One + num_traits::Float + num_traits::FromPrimitive + 'static,
{
    ...
    fn backward(
        ...
        let grad = crate::ops::arithmetic::div(&one, &sqrt_term)?; // 1 / sqrt(...)
...
impl<B, S, T> DifferentiableFunction<B, S, T> for AsinFunction<B, S, T>
where
    B: Backend<Data = T>,
    S: Storage<T> + StorageFromVec<T>,
    T: DataType,
...
impl<B, S, T> crate::AsAny for AsinFunction<B, S, T>
where
    B: Backend<Data = T>,
    S: Storage<T> + StorageFromVec<T>,
    T: DataType,
