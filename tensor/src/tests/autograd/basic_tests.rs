...
fn test_basic_gradient_computation() {
    ...
    let computed_grad = x.grad().unwrap().as_scalar();
...
fn test_multiplication_gradient() {
    ...
    // Validate gradients
    assert_relative_eq!(a.grad().unwrap().as_scalar(), 3.0, epsilon = 1e-6); // ∂f/∂a = b
    assert_relative_eq!(b.grad().unwrap().as_scalar(), 2.0, epsilon = 1e-6); // ∂f/∂b = a
}
...
fn test_scalar_tensor_operations() {
    ...
    assert_relative_eq!(x2.grad().unwrap().as_scalar(), 4.0, epsilon = 1e-6);
}
...
fn test_gradient_reset() {
    ...
    let grad1 = x.grad().unwrap().as_scalar();
    ...
    let grad2 = x2.grad().unwrap().as_scalar();
