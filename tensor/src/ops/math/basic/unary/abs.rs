...
/// Element-wise absolute value
pub fn abs<T, B, S>(tensor: &Tensor<B, S, T>) -> Result<Tensor<B, S, T>>
where
...
{
    let data: Vec<T> = tensor.as_slice().iter().map(|&x| x.abs()).collect();
    let mut result =
    ...
    if crate::tensor_core::grad_enabled() && tensor.requires_grad() {
        let grad_fn = AbsFunction::new(Arc::new(tensor.clone()));
        result = result
            .requires_grad_(true)
