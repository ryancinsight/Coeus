...
use crate::functions::math::SqrtFunction;
...
/// Element-wise square root
pub fn sqrt<T, B, S>(tensor: &Tensor<B, S, T>) -> Result<Tensor<B, S, T>>
where
...
{
    let data: Vec<T> = tensor.as_slice().iter().map(|&x| x.sqrt()).collect();
    let mut result =
    ...
    if crate::tensor_core::grad_enabled() && tensor.requires_grad() {
        let grad_fn = SqrtFunction::new(Arc::new(tensor.clone()));
        result = result
            .requires_grad_(true)
