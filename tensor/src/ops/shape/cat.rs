...
pub fn cat<B, T, S>(tensors: &[Tensor<B, S, T>], dim: usize) -> Result<Tensor<B, S, T>>
where
    ...
    S: Storage<T> + StorageFromVec<T> + StorageToDense<T> + crate::ops::TensorStorageOps<T> + Clone + 'static,
{
    ...
    let mut result =
        Tensor::from_vec_with_backend(result_data, &out_shape, tensors[0].backend.clone())?;

    if crate::tensor_core::grad_enabled() && tensors.iter().any(|t| t.requires_grad()) {
        let arc_inputs: Vec<Arc<Tensor<B, S, T>>> =
            tensors.iter().map(|t| Arc::new(t.clone())).collect();
