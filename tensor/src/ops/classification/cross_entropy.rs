    ...
    S: Storage<T> + StorageToDense<T> + StorageFromVec<T> + crate::ops::TensorStorageOps<T> + Clone + Send + Sync + 'static,
{
    ...
    if is_indices {
        ...
        let log_probs = softmax(input, -1)?
            .as_slice()
            .iter()
            .map(|&x| x.ln())
            .collect::<Vec<_>>();
        let log_probs_tensor =
            Tensor::from_vec_with_backend(log_probs, input_shape, input.backend().clone())?;
        ...
        if crate::tensor_core::grad_enabled() && (input.requires_grad() || target.requires_grad()) {
            let grad_fn =
                CrossEntropyFunction::new(Arc::new(input.clone()), Arc::new(target.clone()));
            result = result
                .requires_grad_(true)
        ...
        let log_softmax = softmax(input, -1)?
            .as_slice()
            .iter()
            .map(|&x| x.ln())
            .collect::<Vec<_>>();
        let log_softmax_tensor: Tensor<B, S, T> =
            Tensor::from_vec_with_backend(log_softmax, input_shape, input.backend().clone())?;
        ...
        let mut result =
            Tensor::from_vec_with_backend(vec![mean_loss], &[1], input.backend().clone())?;
