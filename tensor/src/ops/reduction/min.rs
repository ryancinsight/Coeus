...
use crate::functions::MinFunction;
...
{
    let dims = [dim];
    let mut result = tensor.reduce_generic(Some(&dims), keepdim, |acc, x| if x < acc { x } else { acc }, T::infinity())?;

    if crate::tensor_core::grad_enabled() && tensor.requires_grad() {
        // Create mask for backward pass: 1 where x == min, 0 otherwise
        ...
        let grad_fn = MinFunction::new(Arc::new(tensor.clone()), Arc::new(mask), dim, keepdim);
        result = result
            .requires_grad_(true)
