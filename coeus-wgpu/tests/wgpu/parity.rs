...
// ── Strided (non-contiguous) elementwise parity via Hephaestus strided kernels ──
//
// Each test creates a transposed or sliced non-contiguous layout so the
// non-contiguous dispatch branch in `elementwise_binary`/`elementwise_unary`
// is exercised on the WGPU path.  The reference is the CPU SequentialBackend
// running the same op on the same non-contiguous view (before forcing
// contiguous), matching the semantics of `to_contiguous_on` → op vs direct op.
