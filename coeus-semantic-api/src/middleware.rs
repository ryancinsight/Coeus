...
pub async fn metrics_middleware(request: Request, next: Next) -> Response {
    ...
    // Record metrics
    metrics::counter!("requests_total", "method" => method.to_string(), "status" => status_code.to_string()) += 1;
...
pub async fn rate_limit_middleware(
    ...
    move |request: Request, next: Next| {
        ...
        let mut rate_limits = state.clone().try_write_owned().unwrap_or_else(|_| panic!("Rate limit state poisoned"));

        let info = rate_limits.entry(client_ip.clone()).or_insert_with(|| RateLimitInfo {
            request_count: 0,
