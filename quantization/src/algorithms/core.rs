...
use dtype::DataType;
use storage::{
    DenseStorage, QuantizedStorage, Storage, StorageToDense,
};

use coeus_error::{StorageError as CoeusStorageError, NNError};
 ...
 #[derive(Debug)]
 pub enum QuantizedWeights<T>
 where
     T: DataType,
 {
     /// 4-bit quantized weights
     Bits4(QuantizedStorage<T, 4>),
     /// 8-bit quantized weights
     Bits8(QuantizedStorage<T, 8>),
     /// 16-bit quantized weights
     Bits16(QuantizedStorage<T, 16>),
 }
 
 impl<T> QuantizedWeights<T>
 where
     T: DataType + num_traits::Float + num_traits::FromPrimitive + std::cmp::PartialOrd,
 {
     ...
     pub fn shape(&self) -> &Shape {
         match self {
             Self::Bits4(storage) => storage.shape(),
             Self::Bits8(storage) => storage.shape(),
             Self::Bits16(storage) => storage.shape(),
         }
     }
     ...
     pub fn len(&self) -> usize {
         match self {
             Self::Bits4(storage) => storage.len(),
             Self::Bits8(storage) => storage.len(),
             Self::Bits16(storage) => storage.len(),
         }
     }
     ...
     {
        match self {
            Self::Bits4(storage) => Ok(storage.to_dense().map_err(|e| Error::Storage(CoeusStorageError::Dense(e.to_string())))?),
            Self::Bits8(storage) => Ok(storage.to_dense().map_err(|e| Error::Storage(CoeusStorageError::Dense(e.to_string())))?),
            Self::Bits16(storage) => Ok(storage.to_dense().map_err(|e| Error::Storage(CoeusStorageError::Dense(e.to_string())))?),
        }
     }
        ...
        f64: From<T>,
    {
        match self {
            Self::Bits4(storage) => {
                Self::storage_to_serializable(storage, QuantizationBitwidth::Bits4)
            }
            Self::Bits8(storage) => {
                Self::storage_to_serializable(storage, QuantizationBitwidth::Bits8)
            }
            Self::Bits16(storage) => {
                Self::storage_to_serializable(storage, QuantizationBitwidth::Bits16)
            }
        }
    }

    /// Helper function to convert storage to serializable format
    fn storage_to_serializable<const BITS: usize>(
        storage: &QuantizedStorage<T, BITS>,
    ...
    pub fn from_serializable(serializable: SerializableQuantizedWeights) -> Result<Self>
    where
        T: From<f64>,
    {
    ...
    f64: From<T>,
{
    ...
    pub fn collect(data: &[T]) -> Result<Self> {
        if data.is_empty() {
            return Err(Error::NN(NNError::InvalidParameter("Cannot collect calibration statistics from empty data".to_string())));
        }
