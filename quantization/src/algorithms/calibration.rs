...
use crate::algorithms::core::{CalibrationConfig, CalibrationStats};
    ...
    f64: From<T>,
{
    ...
    pub fn get_optimal_params(&self, layer_name: &str, bits: usize) -> Result<(T, T)> {
        let layer_samples =
            self.layer_stats
                .get(layer_name)
                .ok_or_else(|| Error::NN(NNError::InvalidParameter(format!("No calibration data found for layer: {}", layer_name))))?;

        if layer_samples.is_empty() {
            return Err(Error::NN(NNError::InvalidParameter(format!("No calibration samples found for layer: {}", layer_name))));
        }
    ...
    fn aggregate_stats(&self, samples: &[CalibrationStats<T>]) -> Result<CalibrationStats<T>> {
        if samples.is_empty() {
            return Err(Error::NN(NNError::InvalidParameter("Cannot aggregate empty statistics".to_string())));
        }
        ...
        for (layer_name, serializable_stats_vec) in serializable.layer_stats {
            let stats_vec: Vec<CalibrationStats<T>> = serializable_stats_vec
                ...
                .map(|s| CalibrationStats {
                    min: <T as From<f64>>::from(s.min),
                    max: <T as From<f64>>::from(s.max),
                    mean: <T as From<f64>>::from(s.mean),
                    std: <T as From<f64>>::from(s.std),
                    percentiles: s.percentiles.into_iter().map(<T as From<f64>>::from).collect(),
