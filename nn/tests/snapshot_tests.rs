...
impl TensorSnapshot {
    fn from_tensor(tensor: &Tensor<CpuBackend, DenseStorage<Float32>, Float32>) -> Self {
        Self {
            shape: tensor.shape().dims().to_vec(),
            data: tensor.as_slice().iter().map(|&x| x.to_f64()).collect(),
        }
    }
}
...
fn test_relu_snapshot() {
    let data = vec![
        Float32::new(-2.0), Float32::new(-1.0), Float32::new(0.0),
    ...
    assert_snapshot!("relu_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_sigmoid_snapshot() {
    ...
    assert_snapshot!("sigmoid_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_tanh_snapshot() {
    let data = vec![
        Float32::new(-1.0), Float32::new(0.0), Float32::new(1.0)
    ];
    ...
    assert_snapshot!("tanh_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_gelu_snapshot() {
    ...
    assert_snapshot!("gelu_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_max_pool2d_snapshot() {
    ...
    let tensor = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(data, &[1, 1, 4, 4]).unwrap();

    let result = functional_pooling::max_pool2d(&tensor, (2, 2), Some((2, 2)), (0, 0)).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("max_pool2d_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_avg_pool2d_snapshot() {
    let data = vec![
        Float32::new(1.0), Float32::new(2.0), Float32::new(3.0), Float32::new(4.0),
        Float32::new(4.0), Float32::new(3.0), Float32::new(2.0), Float32::new(1.0),
        Float32::new(1.0), Float32::new(2.0), Float32::new(3.0), Float32::new(4.0),
        Float32::new(4.0), Float32::new(3.0), Float32::new(2.0), Float32::new(1.0),
    ];
    let tensor = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(data, &[1, 1, 4, 4]).unwrap();
    ...
    assert_snapshot!("avg_pool2d_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_linear_snapshot() {
    ...
    let input = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(input_data, &[1, 3]).unwrap();

    let weight_data = vec![
        Float32::new(1.0), Float32::new(0.0), Float32::new(0.0),
        Float32::new(0.0), Float32::new(1.0), Float32::new(0.0),
    ];
    let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(weight_data, &[2, 3]).unwrap();

    let result = functional_linear::linear(&input, &weight, None).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("linear_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_linear_with_bias_snapshot() {
    ...
    let input = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(input_data, &[1, 2]).unwrap();

    let weight_data = vec![
        Float32::new(1.0), Float32::new(2.0),
        Float32::new(3.0), Float32::new(4.0),
    ];
    let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(weight_data, &[2, 2]).unwrap();

    let bias_data = vec![Float32::new(0.5), Float32::new(1.5)];
    let bias = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(bias_data, &[2]).unwrap();

    let result = functional_linear::linear(&input, &weight, Some(&bias)).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("linear_with_bias", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_conv2d_snapshot() {
    ...
    let input = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(input_data, &[1, 1, 3, 3]).unwrap();

    let weight_data = vec![
        Float32::new(1.0), Float32::new(0.0),
        Float32::new(0.0), Float32::new(1.0),
    ];
    let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(weight_data, &[1, 1, 2, 2]).unwrap();

    let result = functional_conv::conv2d(&input, &weight, None, Some((1, 1)), Some((0, 0))).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("conv2d_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_softmax_snapshot() {
    ...
    assert_snapshot!("softmax_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_mse_loss_snapshot() {
    ...
    let pred = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(pred_data, &[3]).unwrap();

    let target_data = vec![Float32::new(1.5), Float32::new(2.5), Float32::new(3.5)];
    let target = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(target_data, &[3]).unwrap();
    ...
    let snapshot = ScalarSnapshot { value: result.as_slice()[0].to_f64() };
    assert_snapshot!("mse_loss_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_layer_norm_snapshot() {
    ...
    let tensor = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(data, &[2, 3]).unwrap();

    let result = functional_normalization::layer_norm(&tensor, &[3], None, None, 1e-5).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("layer_norm_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_layer_norm_with_affine_snapshot() {
    let data = vec![Float32::new(1.0), Float32::new(2.0), Float32::new(3.0)];
    let tensor = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(data, &[1, 3]).unwrap();

    let weight_data = vec![Float32::new(2.0), Float32::new(1.0), Float32::new(0.5)];
    let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(weight_data, &[3]).unwrap();

    let bias_data = vec![Float32::new(0.1), Float32::new(0.2), Float32::new(0.3)];
    let bias = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(bias_data, &[3]).unwrap();

    let result = functional_normalization::layer_norm(&tensor, &[3], Some(&weight), Some(&bias), 1e-5).unwrap();
    let snapshot = TensorSnapshot::from_tensor(&result);

    assert_snapshot!("layer_norm_affine", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_silu_snapshot() {
    ...
    assert_snapshot!("silu_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}

/// Test Leaky ReLU activation snapshot
#[test]
fn test_leaky_relu_snapshot() {
    let data = vec![Float32::new(-2.0), Float32::new(-1.0), Float32::new(0.0), Float32::new(1.0)];
    ...
    assert_snapshot!("leaky_relu_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
...
fn test_elu_snapshot() {
    ...
    assert_snapshot!("elu_basic", serde_json::to_string_pretty(&snapshot).unwrap());
}
