...
impl Arbitrary for ArbitraryTensorData {
...
pub fn fuzz_relu(data: &[u8]) {
    ...
    let tensor = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        float_data, &arb_data.shape
    ) {
        Ok(t) => t,
...
pub fn fuzz_conv2d(data: &[u8]) {
    ...
    for _ in 0..weight_elements {
        weight_data.push(Float32::new(u.f32().unwrap_or(0.0)));
    }

    // Convert data to tensors
    let input_float_data: Vec<Float32> = input_data.data.iter().map(|&x| Float32::new(x)).collect();

    let input = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        input_float_data, &input_data.shape
    ) {
        Ok(t) => t,
        Err(_) => return,
    };

    let weight = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
...
pub fn fuzz_linear(data: &[u8]) {
    ...
    let input = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        input_float_data, &[1, in_features] // Add batch dimension
    ) {
        Ok(t) => t,
        Err(_) => return,
    };

    let weight_float_data: Vec<Float32> = weight_data.iter().map(|&x| Float32::new(x)).collect();
    let weight = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        weight_float_data, &[out_features, in_features]
    ) {
        Ok(t) => t,
        Err(_) => return,
    };
...
pub fn fuzz_mse_loss(data: &[u8]) {
    ...
    let float_data: Vec<Float32> = arb_data.data.iter().map(|&x| Float32::new(x)).collect();
    let pred = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        float_data.clone(), &arb_data.shape
    ) {
        Ok(t) => t,
        Err(_) => return,
    };
    ...
    let target_data: Vec<Float32> = float_data.iter().map(|&x| Float32::new(x + 1.0)).collect();
    let target = match Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::from_vec(
        target_data, &arb_data.shape
    ) {
        Ok(t) => t,
        Err(_) => return,
    };
