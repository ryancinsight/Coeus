...
use super::{VisionLanguageData, ImageTextPair, DatasetSplit, DatasetStatistics};
...
impl Flickr30kDataset {
    ...
    pub async fn with_split(base_path: impl AsRef<Path>, split: DatasetSplit) -> Result<Self> {
        ...
        let (all_annotations, image_id_to_filename) = Self::load_annotations(&annotations_path).await?;
    ...
    async fn load_annotations(captions_path: &Path) -> Result<(HashMap<String, Vec<String>>, HashMap<String, String>)> {
        ...
        for (line_num, line) in content.lines().enumerate() {
            ...
            if image_caption_parts.len() != 2 {
                println!("Warning: Malformed image ID {} on line {}", image_caption_id, line_num + 1);
                continue;
            }
            ...
            annotations.entry(image_filename)
                .or_insert_with(Vec::new)
                .push(caption_text.to_string());
        }

        println!("Loaded annotations for {} unique images", annotations.len());
        println!("Total captions: {}", annotations.values().map(|v| v.len()).sum::<usize>());
    ...
    fn create_train_val_split(image_id_to_filename: &HashMap<String, String>) -> (Vec<String>, Vec<String>) {
        ...
        println!("Created train/val split: {} train, {} validation",
                train_images.len(), val_images.len());

        (train_images, val_images)
    }

    /// Compute dataset statistics
    async fn compute_statistics(annotations: &HashMap<String, Vec<String>>, image_dir: &Path) -> DatasetStatistics {
        ...
        DatasetStatistics {
            ...
            image_sizes: if image_sizes.is_empty() { None } else { Some(image_sizes) },
            disk_size_mb,
        }
    }
    ...
    pub fn clean_captions(&mut self) {
        ...
        // Update statistics
        self.statistics.avg_caption_length = Self::compute_average_caption_length(&self.annotations);
    }
...
mod tests {
    ...
    async fn test_flickr30k_dataset_loading() {
        ...
        let dataset = Flickr30kDataset::with_split(flickr_path, DatasetSplit::Train).await.unwrap();
    ...
    async fn test_caption_cleaning() {
        ...
        let mut dataset = Flickr30kDataset::with_split(flickr_path, DatasetSplit::Train).await.unwrap();
