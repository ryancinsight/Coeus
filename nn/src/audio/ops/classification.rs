...
pub struct AudioClassifier {
    ...
    feature_extractor: ClassificationFeatureType,
...
#[derive(Debug)]
pub enum ClassificationFeatureType {
    /// MFCC features
    MFCC,
    /// Mel spectrogram
    MelSpectrogram,
    ...
    fn get_prediction(&self, probabilities: &[f32]) -> Result<(String, f32)> {
        ...
        if max_prob < self.threshold {
            return Err(NNError::InvalidInput {
                message: format!("Confidence {:.3} below threshold {:.3}", max_prob, self.threshold)
            });
        }
...
impl MusicGenreClassifier {
    ...
    pub fn extract_characteristics(&self, _audio: &[f32], _sample_rate: usize) -> Option<MusicCharacteristics> {
