...
pub struct RealTimeAudioProcessor {
    ...
    feature_extractors: Vec<Box<dyn StreamFeatureExtractor + Send + Sync>>,
    /// Processing state
...
pub trait AudioProcessorStage: std::fmt::Debug {
    /// Process audio data
...
pub trait StreamFeatureExtractor: std::fmt::Debug {
...
impl RealTimeAudioProcessor {
    ...
    pub fn add_feature_extractor(&mut self, extractor: Box<dyn StreamFeatureExtractor + Send + Sync>) {
    ...
    pub fn process(&mut self, audio: &[f32]) -> Result<Vec<f32>> {
        ...
        for stage in &mut self.stages {
            processed = match stage {
                ...
                ProcessingStage::Resampling(_params) => {
