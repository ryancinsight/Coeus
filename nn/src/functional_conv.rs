...
use coeus_tensor::{Tensor, conv_kernel_dot_product_simd, add_bias_simd};
...
///
/// let input = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::ones(&[1, 3, 32, 32]).unwrap();
/// let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::ones(&[64, 3, 3, 3]).unwrap();
///
    ...
    T: Clone + std::ops::Add<Output = T> + std::ops::Mul<Output = T>,
{
    ...
    if weight_shape.len() != 4 {
        return Err(NNError::InvalidInput {
            message: format!("Weight must be 4D [C_out, C_in, K_h, K_w], got {}D", weight_shape.len()),
        });
    }
            ...
            for oh in 0..out_height {
                for ow in 0..out_width {
                        ...
                        for kw in 0..kernel_width {
                            ...
                            if ih >= padding_h && ih < in_height + padding_h
                                ...
                                for ic in 0..in_channels {
                                    ...
                                    input_patch.push(input_data[input_idx].clone());
                                    weight_patch.push(weight_data[weight_idx].clone());
                                }

                                // Use SIMD-accelerated dot product
                                sum = sum + conv_kernel_dot_product_simd(&input_patch, &weight_patch);
                            }
                        }
                    }
                    ...
                    if let Some(bias_tensor) = bias {
                        sum = sum + bias_tensor.as_slice()[oc].clone();
                    }
...
/// use coeus_dtype::float::Float32;
///
/// let input = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::ones(&[1, 64, 16, 16]).unwrap();
/// let weight = Tensor::<CpuBackend, DenseStorage<Float32>, Float32>::ones(&[64, 128, 3, 3]).unwrap();
///
    ...
    T: Clone + std::ops::Add<Output = T> + std::ops::Mul<Output = T>,
{
    ...
    if weight_shape.len() != 4 {
        return Err(NNError::InvalidInput {
            message: format!("Weight must be 4D [C_in, C_out, K_h, K_w], got {}D", weight_shape.len()),
        });
    }
        ...
        for ic in 0..in_channels {
            for ih in 0..in_height {
                for iw in 0..in_width {
                    let input_val = input_data[((b * in_channels + ic) * in_height + ih) * in_width + iw].clone();
                        ...
                        for kw in 0..kernel_width {
                            for oc in 0..out_channels {
                                ...
                                if oh < out_height && ow < out_width {
                                    let weight_idx = ((ic * out_channels + oc) * kernel_height + kh) * kernel_width + kw;
                                    let output_idx = ((b * out_channels + oc) * out_height + oh) * out_width + ow;

                                    output_data[output_idx] = output_data[output_idx].clone() + input_val.clone() * weight_data[weight_idx].clone();
                                }
                            }
                        }
                    }
        ...
        for b in 0..batch_size {
            for oc in 0..out_channels {
                for oh in 0..out_height {
                    for ow in 0..out_width {
                        let output_idx = ((b * out_channels + oc) * out_height + oh) * out_width + ow;
                        output_data[output_idx] = output_data[output_idx].clone() + bias_data[oc].clone();
                    }
                }
            }
        }
