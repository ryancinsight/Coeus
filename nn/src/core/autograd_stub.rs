...
pub enum AutogradError {
    ...
    NotSupportedInTesting { operation: String },
    #[error("Gradient computation failed: {message}")]
    GradientComputationFailed { message: String },
}
...
///
/// Signature matches `autograd::backward` for API compatibility.
pub fn backward<B, S, T>(
    _tensor: &Tensor<B, S, T>,
    _grad_tensor: Option<Tensor<B, S, T>>,
    _retain_graph: bool,
    _create_graph: bool,
) -> Result<()>
where
