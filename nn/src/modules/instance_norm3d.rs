...
//!
//! Applies instance normalization over 3D input tensors.
...
impl<T: FloatDtype> Module<T> for InstanceNorm3d<T> {
    fn forward(&self, input: &Tensor<T>) -> Result<Tensor<T>> {
        // Input shape validation for 3D instance norm: (batch_size, channels, depth, height, width)
        if input.ndim() != 5 {
            return Err(crate::NNError::InvalidInput {
                message: "InstanceNorm3d requires 5D input (batch_size, channels, depth, height, width)"
                    .to_string(),
            });
        }
        ...
        if input_shape[1] != self.num_features {
            return Err(crate::NNError::InvalidInput {
                message: format!(
                    "InstanceNorm3d expected {} channels, got {}",
