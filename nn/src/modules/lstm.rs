...
use crate::Result;
use coeus_tensor::{ops::reduction, FloatDtype, Tensor};
use rand::prelude::*;
...
impl<T: FloatDtype + rand::distributions::uniform::SampleUniform> Lstm<T> {
    ...
    pub fn new(input_size: usize, hidden_size: usize) -> Self {
        ...
        let create_weight = |rng: &mut ThreadRng, shape: Vec<usize>| -> Tensor<T> {
        ...
        let create_bias = |rng: &mut ThreadRng, size: usize| -> Option<Tensor<T>> {
            let data: Vec<T> = (0..size)
                ...
                .collect();
            Some(Tensor::from_vec(data, vec![size]))
        };
    ...
    pub fn forward(
        ...
        // Stack outputs
        let _output_refs: Vec<&Tensor<T>> = outputs.iter().collect();
        let output = if outputs.is_empty() {
            Tensor::zeros(vec![0, batch_size, self.hidden_size])
        } else {
