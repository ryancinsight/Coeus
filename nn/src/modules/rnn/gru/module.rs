    ...
    T: DataType + FloatExt + std::ops::Neg<Output = T>,
{
    fn forward_layer_unidirectional(
        ...
        for t in 0..seq_len {
            ...
            let mut ih_gates = tensor::ops::matmul(&x_t, &weight_ih.transpose(1, 0)?)?;
            let mut hh_gates = tensor::ops::matmul(&current_hidden, &weight_hh.transpose(1, 0)?)?;
