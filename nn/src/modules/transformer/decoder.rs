    ...
    Dropout: Module<B, S, T, Input = Tensor<B, S, T>, Output = Tensor<B, S, T>>,
{
    pub fn new(d_model: usize, nhead: usize, dim_feedforward: usize, dropout: f64) -> Result<Self> {
        ...
        Ok(Self {
            ...
            cross_attn: MultiHeadAttention::<B, S, T>::new(
                d_model, nhead,
            )?,
            ...
            linear2: Linear::<B, S, T>::new(dim_feedforward, d_model)
                .unwrap(),
            norm3: LayerNorm::<B, S, T>::new(vec![d_model], 1e-5),
    ...
    pub fn set_memory(&mut self, memory: Option<Tensor<B, S, T>>) {
        self.memory = memory;
    }
...
impl<B, S, T> Module<B, S, T> for TransformerDecoder<B, S, T>
where
    ...
    Dropout: Module<B, S, T, Input = Tensor<B, S, T>, Output = Tensor<B, S, T>>,
{
    ...
    fn forward(&self, input: &Tensor<B, S, T>) -> Result<Tensor<B, S, T>> {
        if let Some(ref memory) = self.memory {
            self.forward_with_memory(input, memory)
        } else {
            let tgt_output = self.self_attn.forward(input)?;
            let norm1_output = self.norm1.forward(&(input + &tgt_output))?;
            ...
            let ff_output = ff_out_2d.reshape(&[batch as isize, seq as isize, d_model as isize])?;
            self.norm3.forward(&(&norm1_output + &ff_output))
        }
    }
