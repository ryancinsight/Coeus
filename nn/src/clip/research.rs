    ...
    T: crate::dtype::DataType,
{
    ...
    eval_config: crate::clip::validation::ValidationConfig,
}
    ...
    T: crate::dtype::DataType + Send + Sync,
{
    ...
    pub async fn evaluate_config(
        ...
        let model = ClipModel::new(clip_config)?;
        ...
        let report = validator.validate(
            ...
            crate::clip::validation::EvaluationType::Full,
        ).await?;
        ...
        let (vision_config, text_config, projection_dim) = if let Some(ref nas) = config.nas_params {
            (
                crate::clip::VisionConfig {
                    ...
                    mlp_ratio: nas.vision_mlp_ratio,
                },
    ...
    T: crate::dtype::DataType + Send + Sync,
{
    ...
    pub fn new(
        ...
        let evaluator = PerformanceEvaluator::new(
            validation_dataset,
            crate::clip::validation::ValidationConfig::default(),
        );
    ...
    fn sample_parameter<T>(&self, range: &ParameterRange<T>, rng: &mut impl Rng) -> Result<T>
    where
        T: Copy + PartialOrd + std::ops::Add<Output = T> + std::ops::Sub<Output = T> + std::ops::Mul<f64, Output = T> + From<f64>,
    {
        match range {
            ParameterRange::Uniform { min, max } => {
                let t: f64 = rng.gen();
                Ok(*min + T::from(t) * (*max - *min))
            }
