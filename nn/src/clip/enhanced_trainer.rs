...
use crate::optim::{Adam, CosineAnnealingLR};
use crate::optim::optimizer::BaseOptimizer;
    ...
    T: DataType + FloatExt + std::ops::Neg<Output = T> + Send + Sync + Clone + Default,
{
    ...
    fn preprocess_text(&self, tokens: &[u32], masks: &[u32]) -> Result<Vec<u32>> {
    ...
    fn save_checkpoint(&self, name: &str, metrics: &ValidationMetrics) -> Result<()> {
