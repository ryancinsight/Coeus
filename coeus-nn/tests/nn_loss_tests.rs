...
use coeus_nn::{binary_cross_entropy, nll_loss, huber_loss};
...
fn test_binary_cross_entropy() {
    ...
    let pred = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([4], &pred_data), true);
    let target = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([4], &target_data), false);
...
fn test_binary_cross_entropy_clamping() {
    ...
    let pred = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([2], &pred_data), true);
    let target = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([2], &target_data), false);
...
fn test_nll_loss() {
    ...
    let log_probs = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([3, 3], &log_probs_data), true);
    ...
    for i in 0..3 {
        for j in 0..3 {
            let idx = i * 3 + j;
            let expected_g = if j == targets[i] { -1.0 / 3.0 } else { 0.0 };
...
fn test_huber_loss() {
    ...
    let pred = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([3], &pred_data), true);
    let target = Var::new(Tensor::<f64, MoiraiBackend>::from_slice([3], &target_data), false);
