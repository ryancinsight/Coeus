...
fn test_mse_loss_forward() {
    ...
    // Pred: [1.0, 2.0, 3.0]
    let pred = Tensor::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::from_vec_with_backend(
    ...
    // Target: [1.5, 2.5, 3.5]
    let target = Tensor::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::from_vec_with_backend(
...
fn test_cross_entropy_loss_forward() {
    ...
    // Logits: [[1.0, 2.0]]
    let logits = Tensor::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::from_vec_with_backend(
        vec![Float32::new(1.0), Float32::new(2.0)],
        &[1, 2],
        backend.clone()
    ).unwrap();
    ...
    let targets = Tensor::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::from_vec_with_backend(
        vec![Float32::new(1.0)],
        &[1],
        backend.clone()
    ).unwrap();
    ...
    let loss = loss::cross_entropy_loss(&logits, &targets).unwrap();
