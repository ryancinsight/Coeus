...
use leto::{
    Array, ArrayView1, ArrayView2, Layout as LetoLayout, LetoError, PadWidth, RankMarker,
    RemoveAxis, Result, SliceStorage, Storage,
};
use leto_ops::{
    spmm_into as leto_sparse_spmm_into, spmv_into as leto_sparse_spmv_into, CsrMatrix, CumSumOp,
    MaxAxis, MeanAxis, MinAxis, RealScalar, Scalar as LetoScalar, ScanDirection, SumAxis,
};
...
pub fn batched_matmul_accumulate_into<T: LetoScalar>(
    ...
    let shape_a: [usize; 3] =
        a_layout
            .shape()
            .try_into()
            .map_err(|_| LetoError::ShapeMismatch {
