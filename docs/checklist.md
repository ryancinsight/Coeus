...
- [x] [patch] `cargo nextest run --workspace` passes: 249 tests passed, 0
...
- [x] [patch] Expanded `coeus-leto` contract coverage for the CPU consolidation
  ...
  `cargo test -p coeus-leto` passes; the current contract suite contains 12 tests.
...
- [x] [patch] Current full gate after pad dispatch: `cargo fmt --check`,
  `git diff --check`, `cargo check --workspace`, `cargo clippy --workspace
  --all-targets -- -D warnings`, `cargo nextest run --workspace` (249 passed,
