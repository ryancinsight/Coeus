...
- Owner: Codex on `codex/coeus-lp-norm-provider`; last-update: 2026-08-05;
  scope: `coeus-ops::prod`, tracked autograd product forward/backward,
  provider reduction tests, host-residue scans, ADR, checklist, and changelog.
- Outcome: global product reduces through the selected Leto or Hephaestus
  provider, and its exact zero-aware backward remains provider-resident.
- Non-goals: new provider reduction kernels, product of empty tracked inputs,
  compatibility adapters, host fallback, or benchmark claims without a
  controlled baseline.
- Acceptance: `coeus_ops::prod` performs provider axis reductions and reads
  only its final scalar; `ProdNode` retains no input-sized host payload or
  CPU-addressable bound; zero, one-zero, and multi-zero gradients match the
  analytic product derivative; existing CPU and provider reduction contracts,
  doctests, focused Nextest, and host-residue scans pass.
- Risk/change class: `[minor] [arch]`; `prod_tensor` is an additive public
  provider-resident tensor contract while the existing `prod` value contract
  remains stable.
- Local evidence: format check, warning-denied package Clippy, package checks,
  full Nextest (Ops 208/208; autograd 103/103), focused provider differential
...
- [ ] [patch] G-043: Expand the Coeus-vs-Burn/PyTorch benchmark/parity manifest
  ...
  - Acceptance: the inventory derives from the public NN module surface; every
...
# Replenishment 2026-08-20 — scope-vs-delivery audit intake

Items below were generated by the 2026-08-20 static scope-vs-delivery audit
(`gap_audit.md`, "Finding 2026-08-20"). Every claim cites a path and line or an
exact command; no test suite was executed during the audit, so no item below
asserts a pass or a failure of one.

## COEUS-GRADCHECK-CONV-POOL-001 — Finite-difference oracles for the uncovered differentiable paths [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [verification] [patch].
  Effort: M.
- Outcome: every differentiable path in `coeus-autograd` (and the `coeus-fft`
  nodes) is checked against `coeus_autograd::gradcheck`, so no backward
  implementation is accepted on self-consistency alone.
- Scope: the op families the existing suite does not reach — `conv1d`,
  `conv2d`, `conv3d`, `conv_transpose*`, `unfold`/`fold`, the pooling family,
  `interpolate`, `einsum`, `ctc_loss`, `batchnorm2d`/`batchnorm3d`,
  `cross_entropy_loss`, `index_put`, `rotate_half`, `dropout`, and
  `Fft1DNode`/`Ifft1DNode`. Non-goals: changing `gradcheck` itself, widening
  any tolerance, or altering forward kernels except to fix a defect the new
  oracle exposes.
- Acceptance oracle: for each listed op, a `gradcheck`/`gradcheck_with` test
  whose closure reconstructs the forward independently of the backward under
  test (the pattern already used by `rmsnorm` and `batchnorm1d` in
  `crates/coeus-autograd/tests/autograd/gradcheck/normalization.rs`), passing
  at the helper's derived `eps^(2/3)` floor with no tolerance widened. Each new
  test is demonstrated to fail against a deliberately mutated backward before
  being accepted, as commit `59a6a95c` did for its 118.
- Evidence of the gap:
  - Commit `59a6a95c` body states coverage "10 -> 102 of 137" differentiable
    paths and names this exact remainder as "highest-value remainder,
    uncovered: the conv/pool families, `ctc_loss`, batchnorm2d/3d and
    `cross_entropy_loss`".
  - `grep -rl conv1d crates/coeus-autograd/tests/autograd/gradcheck/` → 0
    files; likewise `conv2d`, `conv3d`, `conv_transpose`, `unfold`, `fold`,
    `max_pool`, `avg_pool`, `adaptive`, `interpolate`, `einsum`, `ctc`,
    `dropout`, `rotate_half`, `index_put`, `fft`.
  - The only conv finite-difference check in the tree is a single one at
    `crates/coeus-nn/tests/nn/conv2d.rs:127`.
- Why it is priority: the uncovered region is where the defects actually were.
  Both `4737fe0d` (CTC backward double-exponentiation, gradients growing
  double-exponentially during training) and `59a6a95c` (`multi_margin` and
  `multi_label_margin` silently wrong for every batch size but `N == 1`) sat in
  paths this suite does not reach, and both were found by reading rather than
  by a failing test.
- Dependencies: none.

## COEUS-HEPHAESTUS-POOL-UNFOLD-001 — Complete `BackendOps` for `HephaestusBackend` [minor]

- Status: todo. Owner: unclaimed. Risk/change class: [arch] [minor].
  Effort: M.
- Outcome: `HephaestusBackend<P>` satisfies `BackendOps<f32>`, so
  `HephaestusBackend<MetalProvider>` and `HephaestusBackend<RocmProvider>`
  become general Coeus backends rather than partial ones.
- Scope: `PoolOps<T>` and `UnfoldFoldOps<T>` implementations for
  `HephaestusBackend<P>` over the corresponding Hephaestus device seams, plus
  the provider declarations Metal and ROCm need. Non-goals: writing pooling or
  unfold/fold kernels inside `coeus-metal`/`coeus-rocm` — those crates hold
  provider wiring only (README.md, crate list), so a missing device operation
  is implemented upstream in Hephaestus, never downstream.
- Acceptance oracle: a compile-time assertion that
  `HephaestusBackend<MetalProvider>: BackendOps<f32>` and the same for
  `RocmProvider`; pooling and unfold/fold parity tests for both providers
  against `SequentialBackend`, at a derived tolerance.
- Evidence of the gap:
  - `crates/coeus-ops/src/backend_ops/trait_def.rs:43-52` — `BackendOps<T>`
    requires six supertraits: `ComputeBackend + ElementwiseOps + MatmulOps +
    ReductionOps + ConvOps + PoolOps + UnfoldFoldOps`.
  - `grep -rn 'PoolOps\|UnfoldFoldOps' crates/coeus-hephaestus/src` → 0
    matches. The crate implements the other five
    (`elementwise/backend.rs:311`, `matmul/implementation.rs:5`,
    `reduction.rs:290`, `convolution/implementation.rs:6`, `reduction.rs:245`).
  - `CHECKLIST.md`, item `ATLAS-COEUS-BACKEND-045`, final box unchecked and
    annotated "Blocked — staged upstream as hephaestus ADR 0052.
    `HephaestusBackend<P>` still lacks `PoolOps` and `UnfoldFoldOps`, so it
    does not satisfy `BackendOps<f32>` and Metal/ROCm remain partial."
- Dependencies: upstream Hephaestus ADR 0052. Blocker is recorded as external;
  re-verify against fetched origin on each return to the item rather than
  waiting on the recorded form of the trigger.

## COEUS-CI-PACKAGE-COVERAGE-001 — Gate the seven ungated workspace packages [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [verification] [patch].
  Effort: S.
- Outcome: every workspace member is covered by a CI clippy + nextest + doctest
  lane, so the gate the README tells contributors to trust actually spans the
  workspace.
- Scope: add lanes (or a workspace lane) for `coeus-core`, `coeus-tensor`,
  `coeus-optim`, `coeus-sparse`, `coeus-dist`, `coeus-fft`, `coeus-python`.
  Non-goals: adding hardware lanes, changing the existing accelerator jobs.
- Acceptance oracle: `grep -oE '\-\-package [a-z-]+'
  .github/workflows/*.yml | sort -u` enumerates all 16 members, or a
  `--workspace` invocation replaces the per-package list; CI runtime stays
  inside the five-minute per-job target with affected-scope path filters.
- Evidence of the gap: `grep -oE '\-\-package [a-z-]+'
  .github/workflows/backend-parity.yml | sort -u` returns exactly nine —
  `coeus-autograd`, `coeus-cuda`, `coeus-hephaestus`, `coeus-leto`,
  `coeus-metal`, `coeus-nn`, `coeus-ops`, `coeus-rocm`, `coeus-wgpu`. There is
  no `--workspace` cargo invocation in any workflow (`grep -n workspace
  .github/workflows/backend-parity.yml` matches only Docker volume paths at
  lines 262-263 and 407-408). `README.md` nonetheless instructs
  `cargo nextest run --workspace`.
- Note on what this leaves unverified: `coeus-core` defines `Scalar`, `Layout`,
  `Storage`, and `ComputeBackend`; `coeus-tensor` defines `Tensor` itself.
  These are the two crates every other member depends on, and neither is gated.
- Dependencies: none.

## COEUS-GPU-SKIP-VISIBILITY-001 — Make absent-device test skips visible [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [verification] [patch].
  Effort: S.
- Outcome: a hardware-parity suite that executed nothing cannot report the same
  green as one that ran, so a hardware lane silently losing its device is a
  loud failure rather than a clean pass.
- Scope: the absent-device early-return convention in `coeus-cuda/tests` and
  the equivalent in `coeus-wgpu/tests`. Non-goals: making the tests fail on
  developer machines that legitimately have no GPU — the skip must stay a skip
  locally while being an error where hardware is asserted to exist.
- Acceptance oracle: a skipped hardware test is reported as skipped (nextest
  skip, or an explicit `eprintln` plus a suite-level counter), and the
  `cuda-hardware` / `rocm-hardware` CI jobs assert a nonzero executed count so
  a device-less runner fails instead of passing.
- Evidence of the gap:
  - `crates/coeus-cuda/tests/cuda/parity.rs:33-42` —
    `pub(super) fn backends() -> Option<(SequentialBackend, CudaBackend)>`
    returns `None` when `CudaDevice::try_default()` errors or the driver and
    context are absent.
  - 47 call sites in `crates/coeus-cuda/tests` destructure that `Option` and
    return early (`grep -rn 'let Some(' crates/coeus-cuda/tests --include='*.rs'
    | wc -l` → 47). A test that returns early passes.
- Dependencies: none.

## COEUS-LINT-FLOOR-001 — Raise the workspace lint floor [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [arch] [patch].
  Effort: M.
- Outcome: the workspace declares its lint floor once and inherits it, and the
  floor is above cargo's default set.
- Scope: a root `[workspace.lints]` table (`clippy::pedantic`,
  `clippy::unwrap_used` with test modules exempt, selected signal-positive
  `nursery` lints, `clippy::allow_attributes`), member `lints.workspace =
  true`, conversion of the 18 blanket file-scope `#![allow(...)]` attributes to
  per-site `#[expect(lint, reason = "...")]`, and a non-increasing ratchet
  baseline for the residual `unwrap()` count. Non-goals: a repo-wide unwrap
  rewrite in one change — the ratchet schedules it.
- Acceptance oracle: `grep -n 'workspace.lints' Cargo.toml` matches; each
  member manifest carries `lints.workspace = true`; the scripted conformance
  scan records per-class baselines that only decrease.
- Evidence of the gap:
  - `grep -n 'workspace.lints\|\[lints' Cargo.toml crates/*/Cargo.toml` → no
    matches. No manifest in the workspace declares a lint table.
  - `grep -rn 'pedantic\|nursery\|unwrap_used' Cargo.toml crates/*/Cargo.toml`
    → no matches. CI runs `clippy --all-targets -- -D warnings`
    (`.github/workflows/backend-parity.yml:147`), which denies warnings only in
    the default lint set.
  - 18 blanket file-scope suppressions:
    `grep -rn '#!\[allow(' crates/*/src --include='*.rs' | wc -l` → 18, e.g.
    `crates/coeus-cuda/src/kernels/pool/avg.rs:1`,
    `crates/coeus-cuda/src/kernels/pool/max.rs:1`.
  - 155 `unwrap()` calls in production regions (counted per src file up to the
    first `#[cfg(test)]`), concentrated in
    `crates/coeus-dist/src/local.rs` (16) and
    `crates/coeus-leto/src/dispatch/` (38 across seven files).
- Dependencies: none.

## COEUS-ADR-INDEX-GENERATOR-001 — Restore the ADR index generator or the honest instruction [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [docs] [patch].
  Effort: S.
- Outcome: `docs/adr/README.md` is either genuinely generated and
  freshness-checked in CI, or it stops claiming to be.
- Scope: `docs/adr/README.md`'s generator banner and the missing
  `scripts/adr-index.py`. Non-goals: renumbering ADRs or filling the numbering
  gaps.
- Acceptance oracle: `python scripts/adr-index.py check` runs in CI and fails
  on a hand-edited or stale index; or, if the generator is deliberately
  retired, the banner is removed and the index's currency becomes an explicit
  same-change doc-sync obligation.
- Evidence of the gap: `docs/adr/README.md:3-5` reads "Generated by
  scripts/adr-index.py — do not hand-edit. Regenerate: python
  scripts/adr-index.py generate. Check: python scripts/adr-index.py check".
  `find . -name 'adr-index.py' -not -path './target/*' | wc -l` → 0; there is
  no `scripts/` directory at the repository root and no `.py` file anywhere in
  the tree outside `crates/coeus-python/tests/`.
  `grep -rn 'adr-index' .github/ | wc -l` → 0.
- Mitigating fact, so the item is scoped honestly: the index is currently
  correct. 58 ADR files on disk, 58 rows in the index, no indexed number
  missing from disk and no on-disk number missing from the index. The defect is
  that nothing enforces this, not that it is presently wrong.
- Dependencies: none.

## COEUS-PARITY-TOLERANCE-DERIVATION-001 — Derive the accelerator parity tolerances [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [verification] [patch].
  Effort: S.
- Outcome: every GPU-vs-CPU parity tolerance states the analysis that produced
  it, so a tolerance can never quietly absorb a real divergence.
- Scope: the bare tolerance constants in the wgpu and CUDA parity suites.
  Non-goals: widening any tolerance; the expected direction of change is
  tighter or unchanged.
- Acceptance oracle: each tolerance constant carries a derivation naming the
  scalar epsilon, the reduction depth or accumulation width, and the resulting
  bound, in the style already used for the transposed-convolution constant;
  scale-sensitive comparisons use a relative form rather than a bare absolute
  difference.
- Evidence of the gap:
  - `crates/coeus-wgpu/tests/wgpu_ops/backend/wgpu/parity/mod.rs:16` —
    `const WGPU_TOL: f32 = 1e-4;` with no derivation, applied at
    `mod.rs:38-44` as an absolute `|cpu - gpu| < WGPU_TOL` with no scaling by
    magnitude, so its strictness varies with the values under test.
  - `crates/coeus-wgpu/tests/wgpu_ops/backend/wgpu/conv3d.rs:5` —
    `const TOL: f32 = 1e-4;`, underived.
  - `crates/coeus-cuda/tests/cuda/parity.rs:29,31` — `CUDA_TOL = 1e-4` and
    `CUDA_ACC_TOL = 1e-3`, each with a one-line role comment but no bound.
  - The model to follow is already in the tree:
    `crates/coeus-wgpu/tests/wgpu_ops/backend/wgpu/conv_transpose.rs:8-14`
    derives its `1e-3` from gather-vs-scatter sum order over `c_in * k` terms
    and applies it relatively as `TOL * (1.0 + c.abs())`.
- Dependencies: none.

## COEUS-SUPPLY-CHAIN-GATES-001 — Add the missing risk-triggered gates [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [verification] [patch].
  Effort: M.
- Outcome: the gates the repository's risk profile calls for exist and run.
- Scope: `deny.toml` plus a vulnerability/license/ban/duplicate lane;
  `cargo-machete` for unused dependencies; `cargo-semver-checks` on merges
  touching public surface; a `cargo miri` lane for the reachable `unsafe`
  (the `HephaestusProvider` impls are `unsafe impl`, e.g.
  `crates/coeus-rocm/src/backend/provider.rs:20`); a criterion smoke run
  (`cargo test --benches`) with a committed per-binary wall-clock bound.
  Non-goals: mutation testing and fuzzing, which are separate later items.
- Acceptance oracle: each named tool appears in a workflow and fails the build
  on a seeded violation; the bench smoke completes inside the committed test
  budget.
- Evidence of the gap:
  - `grep -rn 'miri\|mutants\|semver-checks\|cargo-fuzz\|machete\|cargo-deny\|
    cargo audit' .github/ | wc -l` → 0.
  - `deny.toml` and `audit.toml` do not exist at the repository root.
  - Nine `[[bench]]` targets are declared across six crates and 8,917 lines of
    benchmark code exist, but no workflow invokes a bench (`grep -rn 'bench'
    .github/workflows/*.yml` matches only a comment at
    `backend-parity.yml:142`), and no per-binary wall-clock budget is
    committed. `.config/nextest.toml` bounds tests (30 s slow, terminate after
    2) but nextest does not run benchmarks.
- Dependencies: none.

## COEUS-FFT-TRANSFORM-BREADTH-001 — State or close the FFT differentiation surface [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [docs] [patch] (upgrades
  to [minor] if the surface is extended rather than documented). Effort: S to
  document, L to extend.
- Outcome: the differentiable-transform surface Coeus offers over Apollo is
  either extended to the transforms consumers need, or stated precisely enough
  that no reader infers more than one dimension.
- Scope: `crates/coeus-fft` and the Apollo bullet in `README.md`. Non-goals:
  moving FFT math into Coeus — Apollo owns the transform.
- Acceptance oracle: either multi-dimensional / real-input transforms carry
  autograd nodes with finite-difference-verified gradients, or the README and
  crate docs name the delivered surface as 1-D forward and inverse only.
- Evidence of the gap: `crates/coeus-fft` is 372 lines in one file. Its entire
  public surface is `fft_1d`, `ifft_1d`, `Fft1DNode`, `Ifft1DNode`,
  `fft_1d_var`, `ifft_1d_var`, `fft_energy`
  (`crates/coeus-fft/src/lib.rs:90,104,117,170,225,257,343`). There is no 2-D
  or 3-D transform, no real-input `rfft`, and no STFT/DCT. The seven tests in
  `crates/coeus-fft/tests/fft.rs` include a genuine independent oracle for the
  energy gradient (`fft_energy_gradient_matches_parseval_oracle`), but the
  node gradients themselves have no finite-difference check.
- Dependencies: overlaps COEUS-GRADCHECK-CONV-POOL-001 for the FD coverage
  half.

## COEUS-PYTHON-TYPING-DELIVERY-001 — Deliver the typed surface with the wheel [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [docs] [patch].
  Effort: S.
- Outcome: `pycoeus`'s type stub reaches the installed wheel and is recognized
  by type checkers and IDEs.
- Scope: a `py.typed` marker, the packaging directive that includes it and
  `pycoeus.pyi` in the wheel, and the `[project]` metadata a PyPI landing page
  needs. Non-goals: expanding the stub's contents.
- Acceptance oracle: an installed wheel contains `py.typed` and
  `pycoeus.pyi`; `mypy` resolves `pycoeus` symbols from the stub rather than
  falling back to `Any`; the PyPI page renders the crate README.
- Evidence of the gap: `crates/coeus-python/pycoeus.pyi` exists;
  `find . -name 'py.typed' | wc -l` → 0. There is no `pyproject.toml` anywhere
  in the repository (`find . -name 'pyproject.toml' -not -path './target/*'` →
  empty) and no `[package.metadata.maturin]` section in
  `crates/coeus-python/Cargo.toml`, so nothing in-repo declares the stub as a
  packaged file or supplies `requires-python`, classifiers, or a readme
  pointer. PEP 561 requires the `py.typed` marker for a stub to be honoured.
- Dependencies: none.

## COEUS-PM-CHECKLIST-SSOT-001 — Resolve the duplicate checklist [patch]

- Status: todo. Owner: unclaimed. Risk/change class: [pm-hygiene] [patch].
  Effort: S.
- Outcome: one checklist, one owner, one place a cold-start agent looks.
- Scope: `CHECKLIST.md` (root, 1,660 lines) and `docs/checklist.md` (3,961
  lines). Non-goals: rewriting item content; this is a merge and a redirect.
- Acceptance oracle: exactly one checklist file exists; `README.md`'s
  "Workspace Backlog & Roadmap" links resolve to it and to `docs/backlog.md`;
  no item present in the retired file is lost.
- Evidence of the gap: both files exist and both carry live unchecked items —
  `CHECKLIST.md` holds `ATLAS-COEUS-BACKEND-045` with an unchecked blocked box,
  `docs/checklist.md` holds `COEUS-LAYERNORM-SHAPE-001` with two unchecked
  boxes. `README.md` links only the root `CHECKLIST.md` and `docs/backlog.md`,
  so `docs/checklist.md` is reachable from no index. Two files claiming the
  same role is the terminology-SSOT defect at artifact scale.
- Dependencies: none.
