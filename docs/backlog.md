...
- Integrated run `0921b452`: 58/60 pass, 20.822s native execution. CPU scan error propagation is corrected; expanded rejection cases and that fix await native collection. Strict five-package all-target Clippy passes (`test_output/device-output-ownership/test-compilation/commands.json`, unchanged snapshot). CUDA scalar compilation remains an [upstream correction](../../hephaestus/backlog.md#heph-cuda-dense-product-scalars).
...
- lease: integration_judge crates/coeus-core/tests/core_ops/storage/device_outputs.rs crates/coeus-cuda/tests/cuda_ops/device_outputs.rs crates/coeus-wgpu/tests/wgpu_ops/device_outputs.rs 2026-09-08T15:35:18.823965+00:00.
