...
**2026-08-02 manifest update**: `crates/coeus-nn/benches/nn_bench/evidence.tsv`
is now the every-family evidence SSOT. Its focused regression derives the 21
family inventory from `coeus-nn/src/lib.rs` and validates every non-missing
source locator. The first checked inventory records Criterion as 6 present, 11
partial, 3 missing, and 1 inapplicable; Rust contracts as 21 present; Python
...
**2026-08-02 provider-owned initialization update**: exact constant-fill
comparisons and analytically bounded distribution tests cover uniform, normal,
Xavier, and Kaiming initialization against PyTorch. Rust now owns finite-value,
ordered-bound, positive-fan, checked-arithmetic, and rank validation through a
typed fallible contract. CPU storage is initialized directly through Leto;
WGPU, CUDA, ROCm, and Metal dispatch through the generic Hephaestus bridge
selected by backend type, with no accelerator-to-CPU fallback in Coeus. Python
maps domain failures to `ValueError` and provider failures to `RuntimeError`.
Python evidence remains 16 present, 4 partial, 0 missing, and 1 inapplicable;
provider differential evidence and exact-head backend CI are tracked by
`COEUS-RANDOM-INIT-PROVIDER-001` and ADR-0049.
Targeted Miri executes the corrected CPU initialization path successfully, but
