...
fn generate_sparse_matrix(rows: usize, cols: usize, sparsity: f64) -> CsrStorage<Float32> {
    ...
    CsrStorage::new(data, indices, indptr, &[rows, cols])
        .expect("generated CSR matrix must satisfy invariants")
}
...
fn bench_sparse_matmul(c: &mut Criterion) {
    ...
    for &size in [256usize, 512, 1024].iter() {
        for &sparsity in [0.1f64, 0.5, 0.9].iter() {
            ...
            group.bench_function(
                ...
                |b: &mut Bencher| {
                    b.iter(|| {
                        ...
                        black_box(
                            spmm.spmm(&matrix_a, &matrix_b_data, size)
                                .expect("spmm must succeed for valid CSR inputs"),
                        );
                    });
                },
            );
...
fn bench_sparse_gradient_accumulation(c: &mut Criterion) {
    ...
    for &size in [512usize, 1024, 2048].iter() {
        for &sparsity in [0.1f64, 0.5, 0.9].iter() {
            group.bench_function(
                ...
                |b: &mut Bencher| {
                    b.iter(|| {
                        ...
                        .expect("DenseStorage::from_vec must accept matching shapes");

                        let backend = CpuBackend::<Float32>::default();
                        let tensor = Tensor::from_storage(storage, backend).requires_grad_(true);

                        // Accumulate gradients
                        accumulator
                            .accumulate_sparse(&tensor, &tensor)
                            .expect("sparse accumulation must succeed for valid tensors");
                        black_box(());
                    });
                },
            );
                ...
                |b: &mut Bencher| {
                    b.iter(|| {
                        ...
                        let csr_size = sparse_matrix.nnz() * std::mem::size_of::<Float32>()
                            + sparse_matrix.nnz() * std::mem::size_of::<usize>()
                            + ((size + 1) * std::mem::size_of::<usize>());
...
fn bench_sparse_utilities(c: &mut Criterion) {
    ...
    for &size in [512usize, 1024].iter() {
        ...
        group.bench_function(
            format!("format_selection_{}x{}", size, size),
            |b: &mut Bencher| {
                b.iter(|| {
        ...
        group.bench_function(
            format!("memory_savings_calc_{}x{}", size, size),
            |b: &mut Bencher| {
                b.iter(|| {
                    let nnz = dense_data.iter().filter(|&x| !x.is_zero()).count();
        ...
        group.bench_function(
            format!("optimize_storage_{}x{}", size, size),
            |b: &mut Bencher| {
                b.iter(|| {
                    let result =
                        sparse_utils::optimize_gradient_storage(&dense_data, &[size, size]);
                    black_box(result.expect("gradient storage optimization must succeed"));
                });
            },
        );
    }
