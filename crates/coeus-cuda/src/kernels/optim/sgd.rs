...
pub fn launch_sgd_step(
    ...
    if !layouts_fit_cuda(&[param_layout, grad_layout, velocity_layout])
        || !layouts_share_shape(&[param_layout, grad_layout, velocity_layout])
    {
        return false;
    }
...
"#;
        ...
        let Ok(gpu_velocity_layout) = GpuLayoutInfo::try_from(velocity_layout) else {
            return false;
        };
