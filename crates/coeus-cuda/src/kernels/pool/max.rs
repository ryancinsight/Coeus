...
"#,
    ...
    let Some(kernel) = get_or_create_kernel(&key, &cuda_src, "max_pool2d_kernel") else {
        return false;
    };
...
"#,
    ...
    let Some(kernel) = get_or_create_kernel(&key, &cuda_src, "max_pool2d_backward_kernel") else {
        return false;
    };
