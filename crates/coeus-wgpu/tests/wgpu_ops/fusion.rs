...
fn test_wgpu_fusion_parity() {
    ...
    // Transfer to GPU
    let a_gpu = a_cpu.to_backend_on(&seq, &wgpu_b).expect("transfer tensor");
    let b_gpu = b_cpu.to_backend_on(&seq, &wgpu_b).expect("transfer tensor");
    let c_gpu = c_cpu.to_backend_on(&seq, &wgpu_b).expect("transfer tensor");

    // Fused expression on GPU: (a_gpu * b_gpu + c_gpu).relu().sigmoid()
    let expr = (a_gpu.expr() * b_gpu.expr() + c_gpu.expr())
        .relu()
        .sigmoid();
    let out_gpu = evaluate_fused(&expr).expect("evaluate fused expression");

    // Transfer back to CPU
    let out_cpu = out_gpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");

    // Fused expression on CPU
    let expr_cpu = (a_cpu.expr() * b_cpu.expr() + c_cpu.expr())
        .relu()
        .sigmoid();
    let expected_cpu = evaluate_fused_cpu(&expr_cpu, &seq).expect("evaluate fused expression");
...
fn test_wgpu_evaluate_fused_reduce() {
    ...
    let evaluated_cpu = evaluate_fused_cpu(&expr_cpu, &seq).expect("evaluate fused expression");
    ...
    let out_sum_cpu = out_sum_gpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    let expected_sum =
    ...
    let out_mean_gpu =
        coeus_wgpu::evaluate_fused_reduce(&expr_gpu, coeus_ops::ReductionOp::Mean, 1)
            .expect("evaluate fused reduction");
    let out_mean_cpu = out_mean_gpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    ...
    let out_max_gpu = coeus_wgpu::evaluate_fused_reduce(&expr_gpu, coeus_ops::ReductionOp::Max, 1)
        .expect("evaluate fused reduction");
    let out_max_cpu = out_max_gpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    let expected_max =
    ...
    let out_min_gpu = coeus_wgpu::evaluate_fused_reduce(&expr_gpu, coeus_ops::ReductionOp::Min, 1)
        .expect("evaluate fused reduction");
    let out_min_cpu = out_min_gpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    let expected_min =
...
fn test_wgpu_fusion_silu() {
    ...
    let out_gpu = evaluate_fused(&expr_gpu).expect("evaluate fused expression");
    ...
    let expected = evaluate_fused_cpu(&expr_cpu, &seq).expect("evaluate fused expression");
