...
fn test_wgpu_transfers_and_addition() {
    ...
    let a_wgpu = a_seq.to_backend_on(&seq, &wgpu_b).expect("transfer tensor");
    let b_wgpu = b_seq.to_backend_on(&seq, &wgpu_b).expect("transfer tensor");

    assert_eq!(a_wgpu.shape(), &[2, 3]);
    assert_eq!(b_wgpu.shape(), &[2, 3]);

    let c_wgpu = add(&a_wgpu, &b_wgpu).expect("valid addition test shapes");
    let c_seq = c_wgpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
...
fn test_wgpu_backend_ops_unified() {
    ...
    let c = coeus_ops::add(&a, &b, &wgpu_b).expect("evaluate addition");
    let c_cpu = c.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    assert_eq!(c_cpu.as_slice(), &[11.0, 18.0, 33.0, 36.0, 55.0, 54.0]);

    let d = coeus_ops::relu(&a, &wgpu_b).expect("evaluate activation");
    let d_cpu = d.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    ...
    let m2 = Tensor::<f32, WgpuBackend>::from_slice_on(vec![2, 2], &[5.0, 6.0, 7.0, 8.0], &wgpu_b)
        .expect("construct tensor");
    let mr = coeus_ops::matmul(&m1, &m2, &wgpu_b).expect("evaluate matmul");
    let mr_cpu = mr.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    assert_eq!(mr_cpu.as_slice(), &[19.0, 22.0, 43.0, 50.0]);

    let s0 = coeus_ops::sum_axis(&a, 0, &wgpu_b).expect("valid WGPU sum axis");
    let s0_cpu = s0.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
...
fn test_wgpu_tiled_matmul() {
    ...
    let c_seq_res = c_wgpu.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");

    let c_seq_expected = coeus_ops::matmul(&a_seq, &b_seq, &seq).expect("evaluate matmul");
...
fn test_wgpu_cow_semantics() {
    ...
    wgpu_b
        .fill(b.storage_mut().expect("access tensor storage"), 10.0)
        .expect("fill tensor storage");

    let a_cpu = a.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
    let b_cpu = b.to_backend_on(&wgpu_b, &seq).expect("transfer tensor");
