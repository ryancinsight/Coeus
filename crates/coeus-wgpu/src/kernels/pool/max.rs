...
pub fn dispatch_max_pool2d<T: WgpuScalar>(
    ...
    let key = format!("max_pool2d_{}", wgsl_type);
...

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("max_pool2d-compute-pass"),
...
pub fn dispatch_max_pool2d_backward<T: WgpuScalar>(
    ...
    let shader_src = format!(
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {{
            ...
            let my_idx = get_physical_index(in_layout, ni, ci, hi, wi);
                                        ...
                                        if (h_in >= 0 && u32(h_in) < h) {{
                                            for (var jkw: u32 = 0u; jkw < kernel_size; jkw = jkw + 1u) {{
                                                ...
                                                if (w_in >= 0 && u32(w_in) < w) {{
                                                    let input_idx = get_physical_index(in_layout, ni, ci, u32(h_in), u32(w_in));
                                                    let val = input[input_idx];
    ...
    let key = format!("max_pool2d_backward_{}", wgsl_type);
    let pipeline = PIPELINE_CACHE.get_or_create(&key, &ctx.device, &shader_src, "main");
    let bind_group_layout = pipeline.get_bind_group_layout(0);
    let bind_group = ctx.device.create_bind_group(&wgpu::BindGroupDescriptor {
    ...
    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("max_pool2d_backward-compute-pass"),
            timestamp_writes: None,
        });
        compute_pass.set_pipeline(&pipeline);
        compute_pass.set_bind_group(0, &bind_group, &[]);
