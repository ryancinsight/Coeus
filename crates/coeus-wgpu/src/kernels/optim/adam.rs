...
unsafe impl<T: bytemuck::Zeroable> bytemuck::Zeroable for AdamParams<T> {}
unsafe impl<T: bytemuck::Pod> bytemuck::Pod for AdamParams<T> {}

pub fn dispatch_adam_step<T: WgpuScalar + coeus_core::Float>(
    param: &wgpu::Buffer,
    param_layout: &Layout,
    grad: &wgpu::Buffer,
    grad_layout: &Layout,
    ...
    let shader_src = format!(
        ...
        @group(0) @binding(5) var<storage, read> params: AdamParams;
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {{
            ...
            param[off_p] = param[off_p] - params.lr * m_hat / denom;
        }}
        "#,
        wgsl_type = wgsl_type,
        one_val = T::WGSL_ONE,
    ...
    let key = format!("adam_step_{}", wgsl_type);
