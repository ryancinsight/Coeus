...
pub struct RmspropParams<T> {
    pub lr: T,
    pub alpha: T,
    pub eps: T,
}
unsafe impl<T: bytemuck::Zeroable> bytemuck::Zeroable for RmspropParams<T> {}
unsafe impl<T: bytemuck::Pod> bytemuck::Pod for RmspropParams<T> {}

pub fn dispatch_rmsprop_step<T: WgpuScalar + coeus_core::Float>(
    param: &wgpu::Buffer,
    param_layout: &Layout,
    grad: &wgpu::Buffer,
    grad_layout: &Layout,
    ...
    let params_data = RmspropParams { lr, alpha, eps };
    ...
    let shader_src = format!(
        ...
        @group(0) @binding(2) var<storage, read_write> v: array<{wgsl_type}>;
        @group(0) @binding(3) var<storage, read> param_layout: LayoutInfo;
        @group(0) @binding(4) var<storage, read> grad_layout: LayoutInfo;
        @group(0) @binding(5) var<storage, read> v_layout: LayoutInfo;
        @group(0) @binding(6) var<storage, read> params: RmspropParams;
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {{
            ...
            for (var d: u32 = 0u; d < param_layout.ndim; d = d + 1u) {{
                ...
                if (d >= param_layout.ndim - v_layout.ndim) {{
                    let vd = d + v_layout.ndim - param_layout.ndim;
                    if (v_layout.shape[vd] > 1u) {{
                        off_v = off_v + coord * v_layout.strides[vd];
                    }}
                }}
            }}

            if (off_p >= arrayLength(&param) || off_g >= arrayLength(&grad) || off_v >= arrayLength(&v)) {{
                return;
            }}

            let g = grad[off_g];
            let v_val = v[off_v] * params.alpha + ({one_val} - params.alpha) * g * g;
            v[off_v] = v_val;

            let denom = sqrt(v_val) + params.eps;
            param[off_p] = param[off_p] - params.lr * g / denom;
        }}
        "#,
        wgsl_type = wgsl_type,
        one_val = T::WGSL_ONE,
    ...
    let key = format!("rmsprop_step_{}", wgsl_type);
    ...
    let mut encoder = ctx
        ...
        .create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("rmsprop-encoder"),
        });

    {
        let mut compute_pass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
            label: Some("rmsprop-compute-pass"),
