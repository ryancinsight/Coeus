...
fn test_exp_autograd() {
    ...
    let x_val = Tensor::from_slice_on(vec![3], &[0.0f32, 1.0f32, 2.0f32], &backend).expect("valid tensor construction");
    let x = Var::new(x_val, true).expect("valid variable construction");

    let y = exp(&x).expect("valid autograd operation");
    ...
    let grad_out = Tensor::from_slice_on(vec![3], &[1.0f32, 2.0f32, 3.0f32], &backend).expect("valid tensor construction");
...
fn test_exp2_autograd() {
    ...
    let y = exp2(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    assert!((y_slice[0] - 1.0).abs() < 1e-12, "exp2(0) = 1");
    assert!((y_slice[1] - 2.0).abs() < 1e-12, "exp2(1) = 2");
    assert!((y_slice[2] - 4.0).abs() < 1e-12, "exp2(2) = 4");
    let grad_seed = Tensor::from_slice_on(vec![4], &[1.0f64; 4], &backend).expect("valid tensor construction");
    y.backward_with_seed(grad_seed).expect("valid backward propagation");
...
fn test_selu_autograd() {
    ...
    let x = Var::new(Tensor::from_slice_on(vec![5], &data, &backend).expect("valid tensor construction"), true).expect("valid variable construction");
    let y = selu(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    ...
    let grad_seed = Tensor::from_slice_on(vec![5], &[1.0f64; 5], &backend).expect("valid tensor construction");
    y.backward_with_seed(grad_seed).expect("valid backward propagation");
    let gx = x.grad().unwrap();
    for (i, &xi) in data.iter().enumerate() {
        let expected = if xi > 0.0 {
...
fn test_erf_autograd() {
    ...
    let x_val = Tensor::from_slice_on(vec![4], &[0.0f64, 1.0, -1.0, 2.0], &backend).expect("valid tensor construction");
    let x = Var::new(x_val, true).expect("valid variable construction");

    let y = erf(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    ...
    let grad_out = Tensor::from_slice_on(vec![4], &[1.0f64, 1.0, 1.0, 1.0], &backend).expect("valid tensor construction");
    y.backward_with_seed(grad_out).expect("valid backward propagation");
    ...
    for &xi in &[0.5f64, -1.5] {
        let xp = Var::new(Tensor::from_slice_on(vec![1], &[xi + h], &backend).expect("valid tensor construction"), false).expect("valid variable construction");
        let xm = Var::new(Tensor::from_slice_on(vec![1], &[xi - h], &backend).expect("valid tensor construction"), false).expect("valid variable construction");
        let numeric = (erf(&xp).expect("valid autograd operation").tensor.as_slice()[0]
            - erf(&xm).expect("valid autograd operation").tensor.as_slice()[0])
            / (2.0 * h);
...
fn test_erfc_autograd() {
    ...
    let x = Var::new(Tensor::from_slice_on(vec![4], &data, &backend).expect("valid tensor construction"), true).expect("valid variable construction");
    let y = erfc(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    // erfc(x) = 1 - erf(x)
    let erf_ref = [
        0.0f64,
...
fn test_log_autograd() {
    let backend = MoiraiBackend::new();
    let x_val = Tensor::from_slice_on(vec![3], &[1.0f32, 2.0f32, 4.0f32], &backend).expect("valid tensor construction");
    let x = Var::new(x_val, true).expect("valid variable construction");

    let y = log(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
...
fn test_sinh_autograd() {
    ...
    let y = sinh(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    for (i, &xi) in data.iter().enumerate() {
...
fn test_cosh_autograd() {
    ...
    let y = cosh(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    for (i, &xi) in data.iter().enumerate() {
...
fn test_log2_autograd() {
    ...
    let y = log2(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    for (i, &xi) in data.iter().enumerate() {
...
fn test_log10_autograd() {
    ...
    let y = log10(&x).expect("valid autograd operation");
    let y_slice = y.tensor.as_slice();
    for (i, &xi) in data.iter().enumerate() {
