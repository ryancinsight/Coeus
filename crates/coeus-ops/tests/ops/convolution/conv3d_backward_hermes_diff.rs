//! Differential verification for the CPU conv3d backward Hermes dot path.
//!
//! Contiguous, unpadded, unit-stride/unit-dilation conv3d weight-gradient
//! width rows are row-contiguous dot products. CPU `BackendOps::conv3d_backward`
...
#[derive(Clone, Copy)]
struct Conv3dBackwardCase<'a> {
    grad_out: &'a [f32],
    grad_out_shape: [usize; 5],
    input: &'a [f32],
    input_shape: [usize; 5],
    weight: &'a [f32],
...
fn grad_weight_reference(case: &Conv3dBackwardCase<'_>) -> Vec<f32> {
    let [n, c_in, d, h, w] = case.input_shape;
    let [c_out, weight_c_in, kd, kh, kw] = case.weight_shape;
    let [out_n, out_c, d_out, h_out, w_out] = case.grad_out_shape;
            ...
            for ikd in 0..kd {
                for ikh in 0..kh {
                    for ikw in 0..kw {
                                            ...
                                            for ow in 0..w_out {
                                                ...
                                                if w_in >= 0 && (w_in as usize) < w {
                                                    ...
                                                    let input_index = (((ni * c_in + ic) * d
                                                        ...
                                                        * w
                                                        + w_in as usize;
                                                    acc += case.grad_out[go_index]
                                                        * case.input[input_index];
                                                }
                                            }
                                        }
                                    }
                        ...
                        grad_weight[(((oc * c_in + ic) * kd + ikd) * kh + ikh) * kw + ikw] += acc;
                    }
                }
            }
        }
...
fn device_grad_weight<B>(backend: &B, case: &Conv3dBackwardCase<'_>) -> Vec<f32>
where
...
fn assert_close(label: &str, actual: &[f32], expected: &[f32]) {
    ...
    for (index, (&got, &want)) in actual.iter().zip(expected).enumerate() {
        ...
        // tested contiguous case uses at most three products per row across
        // depth, height, and batch accumulation, so this bound covers the
        // reordered row reductions plus outer accumulation.
        let tol = 192.0 * f32::EPSILON * (1.0 + want.abs());
        assert!(
            (got - want).abs() <= tol,
...
where
    ...
    B::DeviceBuffer<f32>: CpuAddressableStorageMut<f32>,
{
    let input: Vec<f32> = (0..180)
        .map(|i| ((i as f32 % 13.0) - 6.0) * 0.0625)
        .collect();
    let weight: Vec<f32> = (0..48).map(|i| ((i as f32 % 7.0) - 3.0) * 0.125).collect();
    let initial_grad_weight: Vec<f32> = (0..48).map(|i| ((i as f32 % 5.0) - 2.0) * 0.01).collect();
    let fast_grad_out: Vec<f32> = (0..48)
        .map(|i| ((i as f32 % 11.0) - 5.0) * 0.03125)
        .collect();
    let fallback_grad_out: Vec<f32> = (0..108)
        .map(|i| ((i as f32 % 17.0) - 8.0) * 0.03125)
        .collect();

    let fast_case = Conv3dBackwardCase {
        grad_out: &fast_grad_out,
        grad_out_shape: [2, 2, 2, 2, 3],
        input: &input,
        input_shape: [2, 2, 3, 3, 5],
        weight: &weight,
        weight_shape: [2, 2, 2, 2, 3],
        initial_grad_weight: &initial_grad_weight,
        stride: 1,
        padding: 0,
        dilation: 1,
    };
    let fallback_case = Conv3dBackwardCase {
        grad_out: &fallback_grad_out,
        grad_out_shape: [2, 2, 3, 3, 3],
        padding: 1,
        dilation: 2,
        ..fast_case
    };
...
#[test]
fn sequential_conv3d_backward_grad_weight_matches_reference() {
    check_backend(&SequentialBackend);
}

#[test]
fn moirai_conv3d_backward_grad_weight_matches_reference() {
