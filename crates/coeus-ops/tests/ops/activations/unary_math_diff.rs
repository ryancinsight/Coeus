    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    // abs: |-3|=3, |-1|=1, |0|=0, |2|=2
    let a = coeus_ops::abs(&v, backend).expect("run operation");
    assert_eq!(a.as_slice(), &[3.0_f64, 1.0, 0.0, 2.0], "abs");

    // neg: 3, 1, 0, -2
    let n = coeus_ops::neg(&v, backend).expect("run operation");
    assert_eq!(n.as_slice(), &[3.0_f64, 1.0, 0.0, -2.0], "neg");

    // sign: -1, -1, 0, 1
    let s = coeus_ops::sign(&v, backend).expect("run operation");
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    let rec = coeus_ops::recip(&r, backend).expect("run operation");
    ...
    let sq = coeus_ops::sqrt(&s, backend).expect("run operation");
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    let fl = coeus_ops::floor(&v, backend).expect("run operation");
    assert_eq!(fl.as_slice(), v.as_slice(), "floor of integers");

    let ce = coeus_ops::ceil(&v, backend).expect("run operation");
    assert_eq!(ce.as_slice(), v.as_slice(), "ceil of integers");

    let ro = coeus_ops::round(&v, backend).expect("run operation");
    ...
    let ro_ties = coeus_ops::round(&ties, backend).expect("run operation");
    assert_eq!(
        ro_ties.as_slice(),
    ...
    let tr = coeus_ops::trunc(&v, backend).expect("run operation");
    ...
    let fl2 = coeus_ops::floor(&frac, backend).expect("run operation");
    assert_eq!(fl2.as_slice(), &[1.0_f64, -2.0], "floor fractions");

    let ce2 = coeus_ops::ceil(&frac, backend).expect("run operation");
    assert_eq!(ce2.as_slice(), &[2.0_f64, -1.0], "ceil fractions");

    let tr2 = coeus_ops::trunc(&frac, backend).expect("run operation");
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    let ez = coeus_ops::exp(&z, backend).expect("run operation");
    assert_eq!(ez.as_slice(), &[1.0_f64], "exp(0)=1");

    // log(1.0) = 0.0 (exact IEEE-754 identity)
    let o = t(&[1], &[1.0], backend);
    let lo = coeus_ops::log(&o, backend).expect("run operation");
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    // sin(0.0) = 0.0 (exact)
    let z = t(&[1], &[0.0], backend);
    let s = coeus_ops::sin(&z, backend).expect("run operation");
    assert_eq!(s.as_slice(), &[0.0_f64], "sin(0)=0");

    // cos(0.0) = 1.0 (exact)
    let c = coeus_ops::cos(&z, backend).expect("run operation");
    assert_eq!(c.as_slice(), &[1.0_f64], "cos(0)=1");

    // tanh(0.0) = 0.0 (exact)
    let t0 = coeus_ops::tanh(&z, backend).expect("run operation");
