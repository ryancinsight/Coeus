    ...
    B: CpuBackend,
{
    ...
    coeus_leto::argmax_into(a_layout, &host_a, axis, c_layout, &mut host_c).map_err(|source| {
    ...
    B: CpuBackend,
{
    ...
    let mut host_c = vec![0i64; c_layout.shape().iter().product()];
    coeus_leto::argmin_into(a_layout, &host_a, axis, c_layout, &mut host_c).map_err(|source| {
        B::Error::from(BackendError::Storage {
            operation: "argmin",
            reason: source.to_string(),
        })
