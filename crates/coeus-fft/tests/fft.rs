...
fn fft_1d_prime_length_matches_dft() {
    ...
    let signal = Tensor::<f64, MoiraiBackend>::from_slice([3], &[1.0, 2.0, 3.0]).expect("construct tensor");
    let spectrum = fft_1d(&signal).expect("FFT succeeds");
    let expected = [
        Complex::new(6.0, 0.0),
        Complex::new(-1.5, s3),
        Complex::new(-1.5, -s3),
...
fn fft_1d_arbitrary_length_roundtrips() {
    ...
    for n in [3usize, 5, 6, 7, 9, 15] {
        ...
        let signal = Tensor::<f64, MoiraiBackend>::from_slice([n], &data).expect("construct tensor");
        let spectrum = fft_1d(&signal).expect("FFT succeeds");
        let recon = ifft_1d(&spectrum).expect("inverse FFT succeeds");
...
fn fft_1d_matches_hand_dft_and_roundtrips() {
    let signal = Tensor::<f64, MoiraiBackend>::from_slice([4], &[1.0, 2.0, 3.0, 4.0]).expect("construct tensor");
    let spectrum = fft_1d(&signal).expect("FFT succeeds");
    let expected = [
    ...
    let reconstructed = ifft_1d(&spectrum).expect("inverse FFT succeeds");
...
fn fft_1d_var_accumulates_input_gradient_from_complex_seed() {
    let x = Var::<f64, MoiraiBackend>::new(Tensor::from_slice([4], &[1.0, 2.0, 3.0, 4.0]).expect("construct tensor"), true).expect("construct variable");
    let y = fft_1d_var(&x).expect("differentiable FFT succeeds");
...
fn fft_1d_f32_matches_hand_dft_and_roundtrips() {
    ...
    let tol = 1e-4_f32;
    let signal = Tensor::<f32, MoiraiBackend>::from_slice([4], &[1.0, 2.0, 3.0, 4.0]).expect("construct tensor");
    let spectrum = fft_1d(&signal).expect("FFT succeeds");
    let expected = [
...
fn fft_1d_f32_agrees_with_f64_reference() {
    ...
    let ref64 = fft_1d(&Tensor::<f64, MoiraiBackend>::from_slice([8], &data).expect("construct tensor"))
        .expect("f64 FFT succeeds");
    let got32 = fft_1d(&Tensor::<f32, MoiraiBackend>::from_slice([8], &data32).expect("construct tensor"))
        .expect("f32 FFT succeeds");
    let tol = 1e-4_f32;
...
fn fft_energy_gradient_matches_parseval_oracle() {
    let data = [0.25, -1.0, 0.5, 2.0, -0.75, 1.25, -0.5, 0.125];
    let x = Var::<f64, MoiraiBackend>::new(Tensor::from_slice([8], &data).expect("construct tensor"), true).expect("construct variable");
    let loss = fft_energy(&x).expect("FFT energy succeeds");
    ...
    loss.backward().expect("FFT energy backward succeeds");
    let grad = x.grad().unwrap();
