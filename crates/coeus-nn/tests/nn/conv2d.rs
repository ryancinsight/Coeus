...
fn test_conv2d_forward_shape() {
    let conv = Conv2d::<f64>::new(3, 8, 3, true).expect("construct module");
    let input = Var::new(Tensor::zeros(vec![2, 3, 32, 32]).expect("construct tensor"), true).expect("construct variable");
...
fn test_conv2d_forward_no_bias() {
    let conv = Conv2d::<f64>::new(1, 1, 3, false).expect("construct module");
    let params = conv.parameters();
...
fn test_conv2d_forward_computation() {
    let mut conv = Conv2d::<f64>::new(1, 1, 2, true).expect("construct module");
...
fn test_conv2d_with_padding() {
    let mut conv = Conv2d::<f64>::with_params(1, 1, 3, 1, 1, 1, true).expect("construct module");
    init::constant(&mut conv.weight, 1.0).expect("initialize parameters");
    if let Some(ref mut b) = conv.bias {
        init::constant(b, 0.0).expect("initialize parameters");
    }

    let input = Var::new(Tensor::zeros(vec![1, 1, 4, 4]).expect("construct tensor"), true).expect("construct variable");
    let output = conv.forward(&input).expect("run forward");
...
fn test_conv2d_with_stride() {
    let mut conv = Conv2d::<f64>::with_params(1, 1, 3, 2, 0, 1, true).expect("construct module");
    init::constant(&mut conv.weight, 1.0).expect("initialize parameters");
    if let Some(ref mut b) = conv.bias {
        init::constant(b, 0.0).expect("initialize parameters");
    }

    let input = Var::new(Tensor::zeros(vec![1, 1, 7, 7]).expect("construct tensor"), true).expect("construct variable");
    let output = conv.forward(&input).expect("run forward");
