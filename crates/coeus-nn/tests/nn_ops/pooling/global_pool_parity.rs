    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    let out1 = Module::<f64, B>::forward(&pool1d, &inp1).expect("valid GlobalAvgPool1d input");
    assert_eq!(
        out1.tensor.shape(),
        &[1, 1, 1],
    ...
    let out1b = Module::<f64, B>::forward(&pool1d, &inp1b).expect("valid GlobalAvgPool1d input");
    assert_eq!(
        out1b.tensor.shape(),
        &[1, 2, 1],
    ...
    let out3 = Module::<f64, B>::forward(&pool3d_avg, &inp3).expect("valid GlobalAvgPool3d input");
    assert_eq!(
        out3.tensor.shape(),
        &[1, 1, 1, 1, 1],
    ...
    let out3m = Module::<f64, B>::forward(&pool3d_max, &inp3).expect("valid GlobalMaxPool3d input");
    assert_eq!(
        out3m.tensor.shape(),
        &[1, 1, 1, 1, 1],
