...
{
    Tensor::from_slice_on(vec![1, 1, l], vals, backend).expect("construct tensor")
}
...
where
    B::DeviceBuffer<f64>: CpuAddressableStorageMut<f64>,
{
    Tensor::from_slice_on(vec![1, 1, h, w], vals, backend).expect("construct tensor")
}
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    let out1 = interpolate_1d(&inp1, 4, InterpolateMode::Nearest).expect("run operation");
    ...
    let out2 = interpolate_1d(&inp2, 2, InterpolateMode::Nearest).expect("run operation");
    assert_eq!(out2.as_slice(), &[2.0_f64, 4.0], "1d nearest downsample");

    // 1-D nearest identity: same L.
    let inp3 = t1d(&[5.0, 3.0, 8.0], 3, backend);
    let out3 = interpolate_1d(&inp3, 3, InterpolateMode::Nearest).expect("run operation");
    ...
    let out4 = interpolate_1d(&inp4, 3, InterpolateMode::Bilinear).expect("run operation");
    ...
    let out5 = interpolate_2d(&inp5, 4, 4, InterpolateMode::Nearest).expect("run operation");
    ...
    let out6 = interpolate_2d(&inp6, 2, 2, InterpolateMode::Nearest).expect("run operation");
    ...
    let out7 = interpolate_2d(&inp7, 2, 2, InterpolateMode::Bilinear).expect("run operation");
