...
pub(crate) fn bench_l1_loss_backward(c: &mut Criterion) {
    ...
    let t_seq = Var::new(
        Tensor::<f32, SequentialBackend>::from_slice(vec![BATCH, FEATURES], &target_data).expect("construct tensor"),
        false,
    ).expect("construct variable");
    ...
    let t_moirai = Var::new(
        Tensor::<f32, MoiraiBackend>::from_slice(vec![BATCH, FEATURES], &target_data).expect("construct tensor"),
        false,
    ).expect("construct variable");
    let mut group = c.benchmark_group("Coeus - l1_loss fwd+bwd (128x256)");
    group.bench_function("Coeus Sequential", |b| {
        b.iter(|| {
            let o = coeus_autograd::l1_loss(black_box(&x_seq), black_box(&t_seq)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.bench_function("Coeus Moirai", |b| {
        b.iter(|| {
            let o = coeus_autograd::l1_loss(black_box(&x_moirai), black_box(&t_moirai)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.finish();
...
pub(crate) fn bench_bce_with_logits_backward(c: &mut Criterion) {
    ...
    group.bench_function("Coeus Sequential", |b| {
        b.iter(|| {
            let o = coeus_autograd::bce_with_logits(black_box(&x_seq), black_box(&t_seq)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.bench_function("Coeus Moirai", |b| {
        b.iter(|| {
            let o = coeus_autograd::bce_with_logits(black_box(&x_moirai), black_box(&t_moirai)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.finish();
...
pub(crate) fn bench_huber_loss_backward(c: &mut Criterion) {
    ...
    group.bench_function("Coeus Sequential", |b| {
        b.iter(|| {
            let o = coeus_autograd::huber_loss(black_box(&x_seq), black_box(&t_seq), 1.0).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.bench_function("Coeus Moirai", |b| {
        b.iter(|| {
            let o = coeus_autograd::huber_loss(black_box(&x_moirai), black_box(&t_moirai), 1.0).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.finish();
...
pub(crate) fn bench_kl_div_backward(c: &mut Criterion) {
    ...
    group.bench_function("Coeus Sequential", |b| {
        b.iter(|| {
            let o = coeus_autograd::kl_divergence(black_box(&x_seq), black_box(&t_seq)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.bench_function("Coeus Moirai", |b| {
        b.iter(|| {
            let o = coeus_autograd::kl_divergence(black_box(&x_moirai), black_box(&t_moirai)).expect("run operation");
            black_box(o).backward().expect("run backward")
        })
    });
    group.finish();
