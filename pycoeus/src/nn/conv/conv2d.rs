...
#[pyclass(name = "Conv2d", module = "coeus.nn", subclass, unsendable)]
#[derive(Clone)]
pub struct PyConv2d {
    pub inner: Conv2DWrapper,
}
...
impl PyConv2d {
    ...
    fn new(
        ...
        let wrapper = match (device, dtype) {
            ("cpu", "float32") => {
                let m = Conv2D::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::new(
                    in_channels,
                    out_channels,
                    k,
                    s,
                ...
                .map_err(to_py_err)?;
                Conv2DWrapper::CpuF32(m)
            }
            ("cpu", "float64") => {
                let m = Conv2D::<CpuBackend<Float64>, DenseStorage<Float64>, Float64>::new(
                    in_channels,
                    out_channels,
                    k,
                    s,
                ...
                .map_err(to_py_err)?;
                Conv2DWrapper::CpuF64(m)
            }
            #[cfg(feature = "gpu")]
            ("cuda", "float32") | ("gpu", "float32") => {
                let m = Conv2D::<GpuBackend<Float32>, DenseStorage<Float32>, Float32>::new(
                    in_channels,
                    out_channels,
                    k,
                    s,
                ...
                .map_err(to_py_err)?;
                Conv2DWrapper::GpuF32(m)
            }
            _ => {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                    "Unsupported device/dtype for Conv2d",
                ))
            }
        };

        Ok(PyConv2d { inner: wrapper })
    }

    #[getter]
    fn weight(&self) -> PyTensor {
        match &self.inner {
            Conv2DWrapper::CpuF32(m) => PyTensor {
                inner: TensorWrapper::CpuDenseF32(m.weight().data().clone()),
            },
            Conv2DWrapper::CpuF64(m) => PyTensor {
                inner: TensorWrapper::CpuDenseF64(m.weight().data().clone()),
            },
            #[cfg(feature = "gpu")]
            Conv2DWrapper::GpuF32(m) => PyTensor {
    ...
    fn bias(&self) -> Option<PyTensor> {
        match &self.inner {
            Conv2DWrapper::CpuF32(m) => m.bias().map(|b| PyTensor {
                inner: TensorWrapper::CpuDenseF32(b.data().clone()),
            }),
            Conv2DWrapper::CpuF64(m) => m.bias().map(|b| PyTensor {
                inner: TensorWrapper::CpuDenseF64(b.data().clone()),
            }),
            #[cfg(feature = "gpu")]
            Conv2DWrapper::GpuF32(m) => m.bias().map(|b| PyTensor {
    ...
    fn forward(&self, input: &PyTensor) -> PyResult<PyTensor> {
        match (&self.inner, &input.inner) {
            (Conv2DWrapper::CpuF32(m), TensorWrapper::CpuDenseF32(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
                    inner: TensorWrapper::CpuDenseF32(res),
                })
            }
            (Conv2DWrapper::CpuF64(m), TensorWrapper::CpuDenseF64(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
            ...
            #[cfg(feature = "gpu")]
            (Conv2DWrapper::GpuF32(m), TensorWrapper::GpuDenseF32(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
    ...
    fn train(&mut self, mode: bool) {
        match &mut self.inner {
            Conv2DWrapper::CpuF32(m) => m.train(mode),
            Conv2DWrapper::CpuF64(m) => m.train(mode),
            #[cfg(feature = "gpu")]
            Conv2DWrapper::GpuF32(m) => m.train(mode),
        }
    }
