use super::{to_py_err, ConvTranspose3DWrapper};
use crate::tensor::{PyTensor, TensorWrapper};
use backend::CpuBackend;
use coeus_nn::core::module::Module;
use coeus_nn::modules::convolution::ConvTranspose3d;
...
#[pyclass(name = "ConvTranspose3d", module = "coeus.nn", subclass, unsendable)]
#[derive(Clone)]
pub struct PyConvTranspose3d {
    pub inner: ConvTranspose3DWrapper,
}
...
impl PyConvTranspose3d {
    ...
    fn new(
        ...
        let op = if let Some(opad) = output_padding {
            if let Ok(op_val) = opad.extract::<usize>() {
                Some((op_val, op_val, op_val))
            } else {
                Some(opad.extract::<(usize, usize, usize)>()?)
            }
        } else {
            None
        };
        ...
        let wrapper = match (device, dtype) {
            ("cpu", "float32") => {
                let m =
                    ConvTranspose3d::<CpuBackend<Float32>, DenseStorage<Float32>, Float32>::new(
                        in_channels,
                        out_channels,
                        k,
                        s,
                    ...
                    .map_err(to_py_err)?;
                ConvTranspose3DWrapper::CpuF32(m)
            }
            ("cpu", "float64") => {
                let m =
                    ConvTranspose3d::<CpuBackend<Float64>, DenseStorage<Float64>, Float64>::new(
                        in_channels,
                        out_channels,
                        k,
                        s,
                    ...
                    .map_err(to_py_err)?;
                ConvTranspose3DWrapper::CpuF64(m)
            }
            #[cfg(feature = "gpu")]
            ("cuda", "float32") | ("gpu", "float32") => {
                let m =
                    ConvTranspose3d::<GpuBackend<Float32>, DenseStorage<Float32>, Float32>::new(
                        in_channels,
                        out_channels,
                        k,
                        s,
                    ...
                    .map_err(to_py_err)?;
                ConvTranspose3DWrapper::GpuF32(m)
            }
            _ => {
                return Err(PyErr::new::<pyo3::exceptions::PyValueError, _>(
                    "Unsupported device/dtype for ConvTranspose3d",
                ))
            }
        };

        Ok(PyConvTranspose3d { inner: wrapper })
    }
    ...
    fn forward(&self, input: &PyTensor) -> PyResult<PyTensor> {
        match (&self.inner, &input.inner) {
            (ConvTranspose3DWrapper::CpuF32(m), TensorWrapper::CpuDenseF32(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
                    inner: TensorWrapper::CpuDenseF32(res),
                })
            }
            (ConvTranspose3DWrapper::CpuF64(m), TensorWrapper::CpuDenseF64(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
            ...
            #[cfg(feature = "gpu")]
            (ConvTranspose3DWrapper::GpuF32(m), TensorWrapper::GpuDenseF32(i)) => {
                let res = m.forward(i).map_err(to_py_err)?;
                Ok(PyTensor {
    ...
    fn train(&mut self, mode: bool) {
        match &mut self.inner {
            ConvTranspose3DWrapper::CpuF32(m) => m.train(mode),
            ConvTranspose3DWrapper::CpuF64(m) => m.train(mode),
            #[cfg(feature = "gpu")]
            ConvTranspose3DWrapper::GpuF32(m) => m.train(mode),
        }
    }
