    ...
    B::DeviceBuffer<f32>: CpuAddressableStorage<f32> + CpuAddressableStorageMut<f32>,
{
    ...
    let c = coeus_ops::einsum("ij,jk->ik", &[&a, &b], backend);
    ...
    let t = coeus_ops::einsum("ij->ji", &[&a2], backend);
    ...
    let tr = coeus_ops::einsum("ii->", &[&sq], backend);
    ...
    let dot = coeus_ops::einsum("i,i->", &[&v1, &v2], backend);
    ...
    let outer = coeus_ops::einsum("i,j->ij", &[&u1, &u2], backend);
    ...
    let mv = coeus_ops::einsum("ij,j->i", &[&mat, &vec_x], backend);
