...
//!   `bmm`           — batched matrix multiply [B,M,K] × [B,K,N] → [B,M,N]
//!
    ...
    B::DeviceBuffer<f64>: CpuAddressableStorage<f64> + CpuAddressableStorageMut<f64>,
{
    ...
    // batch=1 single matmul: [1,2,3] x [1,3,2]
    // A=[[1,0],[0,2],[3,1]], B=[[1,2],[3,0],[0,1]]
    // C[0,0]=1*1+0*3+0*0=1  C[0,1]=1*2+0*0+0*1=2
    // C[1,0]=0*1+2*3+0*0=6  C[1,1]=0*2+2*0+0*1=0? Wait let me recount.
    // A shape [1,3,2], B shape [1,2,3], C shape [1,3,3]
    // A=[[1,0],[0,2],[3,1]], B=[[1,0,0],[0,1,0]]
    // C[0]=1*1+0*0=1, C[0,1]=1*0+0*1=0, C[0,2]=0
    // C[1]=0*1+2*0=0, 0+2=2, 0
    // C[2]=3+0=3, 0+1=1, 0
    // → [[1,0,0],[0,2,0],[3,1,0]]
    // Simpler: use A=I[1,2,2], B=scaling[1,2,2]
