//! Sparse matrix element-wise division trait and implementations
//!
...
pub trait SparseDiv<T: DataType> {
    /// Element-wise divide this matrix by another
    fn div_sparse(&self, other: &Self) -> Result<Self>
    where
        Self: Sized;
}

/// CSR sparse division implementation
///
/// Element-wise division. Division by zero at a position where other is zero
/// but self is non-zero will propagate the NaN/Inf behavior of the type.
impl<T: DataType + core::ops::Div<Output = T> + Copy + PartialEq + num_traits::Zero> SparseDiv<T>
    for CsrStorage<T>
{
    fn div_sparse(&self, other: &Self) -> Result<Self> {
        ...
        for row in 0..rows {
            ...
            // Only matching column indices where self is non-zero produce results
            while a_idx < a_end && b_idx < b_end {
                ...
                match a_col.cmp(&b_col) {
                    core::cmp::Ordering::Less => {
                        // self has value, other is zero -> undefined (skip)
                        a_idx += 1;
                    }
