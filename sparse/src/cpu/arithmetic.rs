...
use crate::Result;
use std::vec::Vec;
use storage::{CooStorage, CscStorage, CsrStorage, SparseFormat, Storage, StorageError};

/// Sparse matrix multiplication trait
pub trait SparseMatMul<T: storage::DataType> {
...
pub trait SparseAdd<T: storage::DataType> {
...
pub trait SparseSub<T: storage::DataType> {
...
pub trait SparseMul<T: storage::DataType> {
...
pub trait SparseDiv<T: storage::DataType> {
...
pub trait SparseElementWise<T: storage::DataType> {
...
pub trait SparseTranspose<T: storage::DataType> {
...
pub trait SparseReshape<T: storage::DataType> {
...
pub trait SparseOptimizerOps<T: storage::DataType> {
...
impl<T: storage::DataType + core::ops::Add<Output = T> + Copy> SparseAdd<T> for CsrStorage<T> {
    fn add_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
...
impl<T: storage::DataType + core::ops::Add<Output = T> + Copy> SparseAdd<T> for CscStorage<T> {
    fn add_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        let other_coo = other.to_coo();
        self_coo.add_sparse(&other_coo)
...
impl<T: storage::DataType + core::ops::Add<Output = T> + Copy> SparseAdd<T> for CooStorage<T> {
    fn add_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        // Validate dimensions
...
impl<T: storage::DataType + core::ops::Sub<Output = T> + core::ops::Neg<Output = T> + Copy>
...
impl<T: storage::DataType + core::ops::Mul<Output = T> + Copy> SparseMul<T> for CsrStorage<T> {
    fn mul_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        let other_coo = other.to_coo();
...
impl<T: storage::DataType + core::ops::Mul<Output = T> + Copy> SparseMul<T> for CscStorage<T> {
    fn mul_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        let other_coo = other.to_coo();
        self_coo.mul_sparse(&other_coo)
...
impl<T: storage::DataType + core::ops::Mul<Output = T> + Copy> SparseMul<T> for CooStorage<T> {
    #[allow(clippy::items_after_statements)]
    fn mul_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        // Validate dimensions
        if self.shape().dims() != other.shape().dims() {
...
impl<T: storage::DataType + core::ops::Div<Output = T> + Copy> SparseDiv<T> for CsrStorage<T> {
    fn div_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        let other_coo = other.to_coo();
...
impl<T: storage::DataType + core::ops::Div<Output = T> + Copy> SparseDiv<T> for CscStorage<T> {
    fn div_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        let other_coo = other.to_coo();
        self_coo.div_sparse(&other_coo)
...
impl<T: storage::DataType + core::ops::Div<Output = T> + Copy> SparseDiv<T> for CooStorage<T> {
    #[allow(clippy::items_after_statements)]
    fn div_sparse(&self, other: &Self) -> Result<CooStorage<T>> {
        // Validate dimensions
        if self.shape().dims() != other.shape().dims() {
...
impl<T: storage::DataType + Copy> SparseElementWise<T> for CsrStorage<T> {
    fn map_nz<F>(&self, op: F) -> Result<Self>
    where
        F: Fn(T) -> T,
    {
...
impl<T: storage::DataType + Copy> SparseElementWise<T> for CscStorage<T> {
    fn map_nz<F>(&self, op: F) -> Result<Self>
    where
        F: Fn(T) -> T,
    {
...
impl<T: storage::DataType + Copy> SparseElementWise<T> for CooStorage<T> {
    fn map_nz<F>(&self, op: F) -> Result<Self>
    where
        F: Fn(T) -> T,
    {
...
impl<T: storage::DataType + Copy> SparseTranspose<T> for CsrStorage<T> {
    fn transpose_sparse(&self) -> Result<CooStorage<T>> {
        let coo = self.to_coo();
...
impl<T: storage::DataType + Copy> SparseTranspose<T> for CscStorage<T> {
    fn transpose_sparse(&self) -> Result<CooStorage<T>> {
        let coo = self.to_coo();
        coo.transpose_sparse()
    }
...
impl<T: storage::DataType + Copy> SparseTranspose<T> for CooStorage<T> {
    fn transpose_sparse(&self) -> Result<CooStorage<T>> {
...
impl<T: storage::DataType + Copy> SparseReshape<T> for CsrStorage<T> {
    fn reshape_sparse(&self, new_shape: &[usize]) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
...
impl<T: storage::DataType + Copy> SparseReshape<T> for CscStorage<T> {
    fn reshape_sparse(&self, new_shape: &[usize]) -> Result<CooStorage<T>> {
        let self_coo = self.to_coo();
        self_coo.reshape_sparse(new_shape)
    }
...
impl<T: storage::DataType + Copy> SparseReshape<T> for CooStorage<T> {
    fn reshape_sparse(&self, new_shape: &[usize]) -> Result<CooStorage<T>> {
...
/// Sparse optimizer arithmetic operations
/// These operations are optimized for optimizer updates on sparse tensors
impl<T: storage::DataType + core::ops::Mul<Output = T> + Copy> SparseOptimizerOps<T>
    for CsrStorage<T>
{
    fn scalar_mul_sparse(&self, scalar: T) -> Result<Self> {
...
mod tests {
    ...
    fn test_coo_sub_sparse() {
        ...
        let row_a = vec![0, 1];
        let col_a = vec![0, 1];
        let coo_storage_a = CooStorage::new(data_a, row_a, col_a, &[2, 2]).unwrap();

        // Matrix B: [[0, 1], [0, 1]]
        let data_b = vec![F32::new(1.0), F32::new(1.0)];
        let row_b = vec![0, 1];
        let col_b = vec![1, 1];
        let coo_storage_b = CooStorage::new(data_b, row_b, col_b, &[2, 2]).unwrap();

        // Result should have 3 elements: A - B
        let result = coo_storage_a.sub_sparse(&coo_storage_b).unwrap();
    ...
    fn test_coo_mul_sparse() {
        ...
        let result = coo_storage_a.mul_sparse(&coo_storage_b).unwrap();
