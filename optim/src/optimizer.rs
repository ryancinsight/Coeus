...
#[derive(Clone, Debug)]
pub struct ParamGroup<T: coeus_dtype::FloatDtype> {
...
impl<T: coeus_dtype::FloatDtype> ParamGroup<T> {
    /// Create a new parameter group
    pub fn new(params: Vec<Tensor<T>>, lr: T, weight_decay: T) -> Self {
        Self {
            params,
            lr,
            weight_decay,
    ...
    pub fn from_params(params: &[Tensor<T>], lr: T, weight_decay: T) -> Self {
    ...
    pub fn parameters(&self) -> &[Tensor<T>] {
        &self.params
    }

    /// Get mutable access to parameters
    pub fn parameters_mut(&mut self) -> &mut [Tensor<T>] {
        &mut self.params
    }
}
...
pub trait Optimizer<T: coeus_dtype::FloatDtype> {
    ...
    fn default_param_group(&self) -> &ParamGroup<T> {
        &self.param_groups()[0]
    }

    /// Get mutable access to the default parameter group
    fn default_param_group_mut(&mut self) -> &mut ParamGroup<T> {
...
pub struct BaseOptimizer<T: coeus_dtype::FloatDtype> {
    /// Parameter groups
    param_groups: Vec<ParamGroup<T>>,
    /// Optimizer state (momentum, variance, etc.)
    state: HashMap<String, Tensor<T>>,
}

impl<T: coeus_dtype::FloatDtype> BaseOptimizer<T> {
    /// Create a new base optimizer
    pub fn new(param_groups: Vec<ParamGroup<T>>) -> Self {
        Self {
            param_groups,
            state: HashMap::new(),
        }
    ...
    pub fn with_defaults(params: Vec<Tensor<T>>, lr: T, weight_decay: T) -> Self {
        let param_group = ParamGroup::new(params, lr, weight_decay);
        Self::new(vec![param_group])
    }
}

impl<T: coeus_dtype::FloatDtype> Optimizer<T> for BaseOptimizer<T> {
    fn name(&self) -> &str {
        "BaseOptimizer"
    }
        ...
        for group in &mut self.param_groups {
            for param in &mut group.params {
                if let Some(grad) = param.grad_mut() {
                    // Zero out the gradient by creating a new zero tensor
                    param.set_grad(Tensor::zeros_like(&grad)).unwrap();
                }
            }
        }
    }
...
pub fn param_requires_grad<T: coeus_dtype::Dtype>(param: &Tensor<T>) -> bool {
    param.requires_grad()
}

/// Helper function to get parameter gradient
pub fn param_grad<T: coeus_dtype::Dtype>(param: &Tensor<T>) -> Option<Tensor<T>> {
    param.grad()
}

/// Helper function to get parameter gradient (now returns owned copy for thread safety)
pub fn param_grad_mut<T: coeus_dtype::Dtype>(param: &Tensor<T>) -> Option<Tensor<T>> {
    param.grad_mut()
}
