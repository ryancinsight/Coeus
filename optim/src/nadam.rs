    ...
    T: DataType + FloatExt + num_traits::Float + num_traits::FromPrimitive,
{
    pub fn new(params: Vec<tensor::Tensor<B, S, T>>, lr: f64) -> Self {
        ...
        Self::with_hyperparams(params, lr, 0.9, 0.999, 1e-8, 0.0, 0.004)
    }
    ...
    pub fn with_hyperparams(
        ...
        assert!(
            momentum_decay >= 0.0,
            "momentum_decay must be non-negative, got {}",
            momentum_decay
        );
...
impl<B, S, T> BaseOptimizer<B, S, T> for NAdam<B, S, T>
where
    B: Backend<Data = T> + Clone + Default,
    S: Storage<T> + Clone + StorageFromVec<T> + 'static + StorageToDense<T> + tensor::ops::arithmetic::traits::TensorStorageArithmetic<T>,
    T: DataType + FloatExt + num_traits::Float + num_traits::FromPrimitive,
{
    ...
    fn step_cpu(&mut self) -> Result<usize, crate::OptimError> {
        ...
        let mu_t_f64 = self.beta1 * (1.0 - 0.5 * 0.96_f64.powf(t_val * self.momentum_decay));
        let mu_tp1_f64 =
            self.beta1 * (1.0 - 0.5 * 0.96_f64.powf((t_val + 1.0) * self.momentum_decay));
        let prod_mu_t_f64 = self.mu_product * mu_t_f64;
        let prod_mu_tp1_f64 = prod_mu_t_f64 * mu_tp1_f64;
        self.mu_product = prod_mu_t_f64;

        let mu_t = T::from(mu_t_f64).unwrap();
        let mu_tp1 = T::from(mu_tp1_f64).unwrap();
        let prod_mu_t = T::from(prod_mu_t_f64).unwrap();
        let prod_mu_tp1 = T::from(prod_mu_tp1_f64).unwrap();
    ...
    fn state_dict(&self) -> std::collections::HashMap<String, tensor::Tensor<B, S, T>> {
        ...
        let mu_product_tensor =
            Tensor::from_vec(vec![T::from(self.mu_product).unwrap()], &[1]).unwrap();
        state.insert("mu_product".to_string(), mu_product_tensor);
        let momentum_decay_tensor =
            Tensor::from_vec(vec![T::from(self.momentum_decay).unwrap()], &[1]).unwrap();
        state.insert("momentum_decay".to_string(), momentum_decay_tensor);
        state
    }
    ...
    fn load_state_dict(
        ...
        if let Some(mu_product_tensor) = state_dict.get("mu_product") {
            if let Some(&mu_product_val) = mu_product_tensor.as_slice().first() {
                self.mu_product = mu_product_val.to_f64().unwrap_or(1.0);
            }
        }
        if let Some(momentum_decay_tensor) = state_dict.get("momentum_decay") {
            if let Some(&momentum_decay_val) = momentum_decay_tensor.as_slice().first() {
                self.momentum_decay = momentum_decay_val.to_f64().unwrap_or(self.momentum_decay);
            }
        }
        Ok(())
    }
...
impl<B, S, T> Optimizer<B, S, T> for NAdam<B, S, T>
where
    B: Backend<Data = T> + Clone + Default,
    S: Storage<T> + Clone + StorageFromVec<T> + 'static + StorageToDense<T> + tensor::ops::arithmetic::traits::TensorStorageArithmetic<T>,
    T: DataType + FloatExt + num_traits::Float + num_traits::FromPrimitive,
