...
async fn run_fft_benchmarks() {
    ...
    for &size in &sizes {
        ...
        for _ in 0..100 {
            // 100 iterations for more stable measurement
        ...
        println!(
            "CPU FFT:   {:.2} ms (avg per transform)",
        ...
        if let Ok(backend) = GpuBackend::new().await {
            ...
            println!(
                "GPU FFT:   {:.2} ms (avg per transform)",
    ...
    let test_size = 1024;
    let original: Vec<f32> = (0..test_size).map(|i| (i as f32 * 0.01).sin()).collect();
