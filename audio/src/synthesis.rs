...
impl TTSEngine {
    ...
    pub fn set_default_model(&mut self, model_name: &str) -> Result<()> {
        let index = self
            ...
            .ok_or_else(|| AudioError::InvalidConfiguration {
    ...
    pub fn synthesize(
        ...
        let model = if let Some(voice_name) = voice {
            self.models
                .iter()
                .find(|m| m.name == voice_name)
                .ok_or_else(|| AudioError::InvalidConfiguration {
                    message: format!("Voice '{}' not found", voice_name),
                })?
        } else {
        ...
        let default_options = SynthesisOptions::default();
        let effective_options = options.unwrap_or(&default_options);
        ...
        let audio = match model.model_type {
            ...
            TTSModelType::FastSpeech2 => {
                self.synthesize_fastspeech2(text, model, effective_options)?
            }
    ...
    pub fn supports_language(&self, language: &str) -> bool {
        self.models
            .iter()
            .any(|m| m.languages.contains(&language.to_string()))
    }
    ...
    fn synthesize_tacotron2(
        ...
        let duration_samples =
            (text.len() as f32 * self.audio_config.sample_rate as f32 * 0.1) as usize;
    ...
    fn synthesize_fastspeech2(
        ...
        let duration_samples =
            (text.len() as f32 * self.audio_config.sample_rate as f32 * 0.08) as usize;
    ...
    fn synthesize_vits(
        ...
        // Simulate audio generation
        let duration_samples =
            (text.len() as f32 * self.audio_config.sample_rate as f32 * 0.12) as usize;
    ...
    fn synthesize_custom(
        ...
        // Simulate audio generation
        let duration_samples =
            (text.len() as f32 * self.audio_config.sample_rate as f32 * 0.15) as usize;
