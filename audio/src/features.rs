...
impl MFCCExtractor {
    ...
    fn init_mel_filters(&mut self) {
        let fmax = self
            .config
            .fmax
            .unwrap_or(self.config.sample_rate as f32 / 2.0);
        ...
        for m in 0..self.num_mels {
            let mel_low =
                mel_fmin + (mel_fmax - mel_fmin) * (m as f32) / (self.num_mels as f32 + 1.0);
            let mel_center =
                mel_fmin + (mel_fmax - mel_fmin) * (m as f32 + 1.0) / (self.num_mels as f32 + 1.0);
            let mel_high =
                mel_fmin + (mel_fmax - mel_fmin) * (m as f32 + 2.0) / (self.num_mels as f32 + 1.0);
...
impl AudioFeatureExtractor for MFCCExtractor {
    fn extract(&self, audio: &[f32], _sample_rate: usize) -> Result<Vec<Vec<f32>>> {
        ...
        let frames = Self::frame_audio(
            &emphasized,
            extractor.config.window_size,
            extractor.config.hop_size,
        );

        // Compute STFT for each frame (placeholder)
        let power_spectra: Vec<Vec<f32>> = frames
            .iter()
            .map(|frame| {
...
impl AudioFeatureExtractor for MelSpectrogramExtractor {
    fn extract(&self, audio: &[f32], _sample_rate: usize) -> Result<Vec<Vec<f32>>> {
        if audio.is_empty() {
            return Err(AudioError::InvalidInput {
                message: "Audio input cannot be empty".to_string(),
        ...
        let frames = MFCCExtractor::frame_audio(
            audio,
            extractor.config.window_size,
            extractor.config.hop_size,
        );
...
impl AudioFeatureExtractor for SpectrogramExtractor {
    fn extract(&self, audio: &[f32], _sample_rate: usize) -> Result<Vec<Vec<f32>>> {
        if audio.is_empty() {
            return Err(AudioError::InvalidInput {
                message: "Audio input cannot be empty".to_string(),
        ...
        // Framing and windowing (simplified)
        let frames =
            MFCCExtractor::frame_audio(audio, self.config.window_size, self.config.hop_size);
...
impl AudioFeatureExtractor for ChromagramExtractor {
    fn extract(&self, audio: &[f32], _sample_rate: usize) -> Result<Vec<Vec<f32>>> {
        if audio.is_empty() {
            return Err(AudioError::InvalidInput {
                message: "Audio input cannot be empty".to_string(),
...
impl AudioFeatureExtractor for WaveletExtractor {
    fn extract(&self, audio: &[f32], _sample_rate: usize) -> Result<Vec<Vec<f32>>> {
        if audio.is_empty() {
            return Err(AudioError::InvalidInput {
                message: "Audio input cannot be empty".to_string(),
...
impl fmt::Display for AudioConfig {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "AudioConfig(sample_rate={}, window={}, hop={}, fft={})",
