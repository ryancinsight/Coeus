...
fn test_cuda_adamw_step() {
    ...
    if coeus_cuda::CudaDriver::get().is_some() && coeus_cuda::get_cuda_context().is_some() {
        for (i, (&res, &exp)) in param_cuda_on_cpu.as_slice().iter().zip(param_seq.as_slice().iter()).enumerate() {
            assert!((res - exp).abs() < 1e-5, "Param mismatch at {}: {} vs expected {}", i, res, exp);
        }
        for (i, (&res, &exp)) in m_cuda_on_cpu.as_slice().iter().zip(m_seq.as_slice().iter()).enumerate() {
            assert!((res - exp).abs() < 1e-5, "M mismatch at {}: {} vs expected {}", i, res, exp);
        }
        for (i, (&res, &exp)) in v_cuda_on_cpu.as_slice().iter().zip(v_seq.as_slice().iter()).enumerate() {
            assert!((res - exp).abs() < 1e-5, "V mismatch at {}: {} vs expected {}", i, res, exp);
        }
    }
}
