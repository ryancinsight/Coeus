...
pub fn launch_adamw_step(
    ...
    if is_contiguous {
        let cuda_src = r#"
extern "C" __global__ void adamw_contiguous_kernel(
    float* param,
    const float* grad,
    float* m,
    float* v,
    ...
    float adam_update = lr * m_hat / denom;
    float wd_update = lr * weight_decay * param[idx];
    param[idx] -= adam_update + wd_update;
}
"#;
        let Some(kernel) = crate::kernels::fuse::get_or_create_kernel("adamw_contiguous", cuda_src, "adamw_contiguous_kernel") else {
            return false;
        };
        ...
        let mut args: [*mut std::ffi::c_void; 12] = [
            ...
            &mut wd_val as *mut f32 as *mut std::ffi::c_void,
...
extern "C" __global__ void adamw_strided_kernel(
    float* param,
    ...
    float denom = sqrtf(v_hat) + eps;
    float adam_update = lr * m_hat / denom;
    float wd_update = lr * weight_decay * param[off_p];
    param[off_p] -= adam_update + wd_update;
}
"#;
        let Some(kernel) = crate::kernels::fuse::get_or_create_kernel("adamw_strided", cuda_src, "adamw_strided_kernel") else {
            return false;
        };

        let gpu_param_layout = GpuLayoutInfo::from_layout(param_layout);
        let gpu_grad_layout = GpuLayoutInfo::from_layout(grad_layout);
        let gpu_m_layout = GpuLayoutInfo::from_layout(m_layout);
