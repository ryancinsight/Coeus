...
extern "C" __global__ void max_pool2d_kernel(
    ...
    unsigned int w_out = output_layout.shape[3];
    unsigned int h_out = output_layout.shape[2];
    ...
    unsigned int ci = temp2 % c;
    unsigned int ni = temp2 / c;

    unsigned int h_in_limit = input_layout.shape[2];
    unsigned int w_in_limit = input_layout.shape[3];
        ...
        if (h_in >= 0 && h_in < (int)h_in_limit) {{
            for (unsigned int ikw = 0; ikw < kernel_size; ++ikw) {{
                ...
                if (w_in >= 0 && w_in < (int)w_in_limit) {{
                    unsigned int input_idx = get_physical_index(input_layout, ni, ci, (unsigned int)h_in, (unsigned int)w_in);
    ...
    unsigned int output_idx = get_physical_index(output_layout, ni, ci, oh, ow);
...
"#,
    ...
    let key = format!("max_pool2d_{}", cuda_type);
    let Some(kernel) = get_or_create_kernel(&key, &cuda_src, "max_pool2d_kernel") else { return false; };
...
extern "C" __global__ void max_pool2d_backward_kernel(
    ...
    unsigned int w = grad_input_layout.shape[3];
    unsigned int h = grad_input_layout.shape[2];
    ...
    unsigned int my_idx = get_physical_index(input_layout, ni, ci, hi, wi);
    ...
    unsigned int h_out_limit = grad_out_layout.shape[2];
    unsigned int w_out_limit = grad_out_layout.shape[3];
                ...
                for (unsigned int ikw = 0; ikw < kernel_size; ++ikw) {{
                    ...
                    if (numer_w >= 0 && numer_w % stride_s == 0) {{
                        ...
                        if (ow < w_out_limit) {{
                            ...
                            if (has_val && max_h == hi && max_w == wi && my_val == max_val) {{
                                unsigned int go_idx = get_physical_index(grad_out_layout, ni, ci, oh, ow);
                                sum += grad_out[go_idx];
                            }}
                        }}
                    }}
    ...
    unsigned int gi_idx = get_physical_index(grad_input_layout, ni, ci, hi, wi);
...
"#,
    ...
    let key = format!("max_pool2d_backward_{}", cuda_type);
    let Some(kernel) = get_or_create_kernel(&key, &cuda_src, "max_pool2d_backward_kernel") else { return false; };
